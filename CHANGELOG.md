...
### Infrastruktur
- **Cluster-Health-Wächter (A3b, `private_k8s/cluster-health-watch/`)** — es gab keine Cluster-Überwachung; CronJob (10 min, reine Stdlib, lesende RBAC) meldet ungesunde CNPG-Cluster über beide Instanzen.
- **Alte `renfield-pg`-CNPG-Cluster gelöscht** — durch `renfield-pg-r1` ersetzt; 60 GiB frei.
- **xidra-Deployment-Konfiguration ins Repo `x-ren` überführt (#1234)** — lag im gitignorierten `k8s/xidra/`; Renfield-Repo trägt nur noch einen Wegweiser.
- **Paperless-Dedupe-Intervall 300 → 3600 s** auf beiden Instanzen.
- **Harbor push/pull vom Heim-Netz: WAN-Hairpin behoben.** Ursache war NICHT MTU (0-Retransmit-Nachmessung), sondern dass LAN-Hosts die Registry auf die Public-IP auflösten und über den WAN-Upload-Cap (~72 Mbit/s) hairpinten. Fix: per-Node-`/etc/hosts`-Pin auf den internen HAProxy-Pfad (`192.168.1.1`) → ~200 Mbit/s (100 MB Push 11 s → 3 s); idempotentes Ansible-Playbook (`private_k8s/ansible/`). Analyse: `docs/TECHNICAL_DEBT.md` I1 + `public_k8s/docs/harbor-slow-from-home-lan.md`.

### Hinzugefuegt
- **Alarme + Funktionssonden (A1–A3, #1230/#1231/#1232)** — schließt die Lücke aus dem 21,5-h-Ausfall vom 2026-09-11: Alarmierung war *an* und schwieg, weil nur die MCP-Prozess-Verbindung geprüft wurde. **A1 Funktionssonden** (`MCP_HEALTH_PROBE_ENABLED`): pro Server ein billiger lesender Aufruf, der gelingen MUSS — sein Scheitern IST ein Gesundheitssignal (`health_probe:`-Stanza, Verdikt `probe_failed`; paperless/n8n/homeassistant; `search` behält seine eigene Sonde). **A2 Fehlserien-Alarm** (`SCHEDULED_TASK_FAILURE_ALERT_ENABLED`, Migration `pc20260912_taskalert`): nach 3 Fehlläufen in Folge genau EINE Meldung, Erholungsmeldung inklusive. **A3 gegenseitiger Wächter** (`WATCHDOG_ENABLED`/`WATCHDOG_TARGETS`): jede Instanz prüft `/health/ready` der anderen, nutzt A2 zum Alarmieren. Geteilter Alarmweg `services/ops_alert.py`. LIVE beide Instanzen.
- **Fix: KG-Reconciler Advisory-Lock-Crash bei Prod-Aktivierung.** Beim ersten `KG_RECONCILER_ENABLED=true` stürzte jeder Lauf ab (`greenlet_spawn has not been called` / `'NoneType' object has no attribute 'cursor'`, im Scheduler abgefangen → No-op). Ursache: die dedizierte Advisory-Lock-Verbindung wurde via `self.db.bind.engine` geöffnet; unter der Prod-Topologie (`AsyncSession` an einer `AsyncEngine`) liefert `.engine` die **synchrone** Engine, deren `.connect()` unter `async with` explodiert. Das Test-Fixture bindet eine `AsyncConnection` (deren `.engine` async ist), daher nie gefangen. Fix: `_resolve_lock_engine(bind)` löst die AsyncEngine typ-korrekt auf (AsyncEngine direkt; AsyncConnection→`.engine`; sonst None→ungelockt als sicherer Fallback). 20/20 PG (inkl. Topologie-Regressionstest).
- **KG-Reconciler Person-Guard** (macht `KG_RECONCILER_ENABLED` sicher aktivierbar). Der Reconciler-Self-Join betraf bisher alle Entitaeten inkl. Personen; da distinkte Personennamen intrinsisch ≥ Kandidaten-Schwelle embedden (Erika~Anna 0.894), konnte ein Personen-Paar bei ≥ Auto-Merge-Schwelle (0.95) ZWEI VERSCHIEDENE Personen still zusammenfuehren. Fix (spiegelt resolve's Personen-Embedding-Skip): ein Personen-beteiligtes Paar (primaer ODER Multi-Typ) mit UNVERWANDTEN Namen wird komplett verworfen — kein Merge, kein Proposal. Personen reconcilen nur bei VERWANDTEN Namen (gleich oder Token-Teilmenge: "Alice" ⊆ "Alice B.", "Erika" ⊆ "Erika van den Muster"). Das Auto-Merge-Gate verlangt die Namensverwandtschaft fuer Personen zusaetzlich (Defense-in-Depth hinter dem Find-Drop), sodass eine Erkennungsluecke nie still zwei Personen verschmilzt. Nicht-Personen unveraendert. 19/19 PG-Tests (inkl. Vulnerability-Test + Gate-Bypass-Test).
- **Fix: WebSocket-Chat-Crash (`UnboundLocalError: asyncio`).** Ein funktions-lokales `import asyncio` in einem Branch von `websocket_endpoint` (`api/websocket/chat_handler.py`) machte `asyncio` zur LOKALEN Variable der ganzen Funktion — auf jedem Pfad, der diesen Branch ueberspringt, warf der spaetere `asyncio.*`-Aufruf `UnboundLocalError` und der Chat-WebSocket stuerzte ab. `asyncio` ist auf Modulebene importiert; der lokale Import wurde entfernt. Regressions-Guard `tests/backend/test_chat_handler_asyncio_scope.py` (AST-Check: kein funktions-lokaler asyncio-Import + Modul-Import vorhanden).
- **KG-Konflations-Tripwire** (`KG_CONFLATION_MONITOR_ENABLED`, opt-in, read-only). Fruehwarnung gegen kuenftige Magnet-Hubs: ein periodischer Per-User-halfvec-Self-Join meldet (WARNING-Log + Prometheus-Gauge `renfield_kg_conflation_candidates`) distinct-name, same-type, same-tier **NICHT-Personen**-Paare, die ≥ `KG_CONFLATION_MONITOR_THRESHOLD` embedden — also zwei Dinge, die verschieden sein sollten, aber gleich aussehen. **Personen sind ausgeschlossen** (primaer ODER Multi-Typ): Personennamen clustern intrinsisch ≥ Schwelle name-only (gemessen Erika~Anna 0.894) und `resolve_entity` matched Personen ohnehin nicht per Embedding — ein nahes Personen-Paar kann nicht falten, ein Treffer waere Dauer-Rauschen. Erwarteter Wert 0; mutiert nie (echte Dubletten = Reconciler). On-demand `bin/scan_kg_conflation.py`. `services/kg_conflation_monitor.py`, im Scheduler (`_schedule_kg_conflation_monitor`). 8/8 PG-Tests.
- **Fix: Personen-Konflation an der Wurzel (Live-Extraktionspfad).** Die eigentliche Ursache der Erika→Anna-Konflation: die Entity-Embeddings werden aus `name + description` gebildet, und der Extraktor fuellte `description` mit einer generischen Typ-Floskel ("Vollständiger Name einer Person"). Das kollabiert jede Personen-Zeile zu einem *Generischen-Personen-Centroid*, von dem aus jeder bloße Vorname ≥ Schwelle landet — so wurde Entitaet #11 zum 127-Mentions-Magnet-Hub (auch #9 "Thomas Meunier", 36). Drei Hebel: (1) **PERSON-Entitaeten ueberspringen den Embedding-Match komplett** (Gate auf dem Multi-Typ-`seed_types`) — Personen werden ueber Name (exakt + Surface-Form) identifiziert, echte Near-Dupes gehen an den review-gated Reconciler; Nicht-Personen behalten den Embedding-Match (OCR/Tippfehler-Varianten Bnn→Bonn). (2) **Extraktions-Prompt** (`prompts/knowledge_graph.yaml`, alle 4 Varianten DE/EN × Dialog/Dokument) verlangt eine entity-SPEZIFISCHE Beschreibung oder leer, nie eine Typ-Meta-Floskel. (3) **Server-seitige Durchsetzung**: `resolve_entity` entfernt generische Beschreibungen (`is_generic_person_description`, **ganz-String-Match**) von Personen-Zeilen vor Embed/Store. (4) **De-Magnetisierungs-Backfill** `services/kg_demagnetize.py` (+ `bin/demagnetize_person_entities.py`, `--dry-run`/`--apply` mit fail-closed Pre-Mutations-Audit-Dump) repariert bestehende Magnet-Zeilen: generische Description → NULL + Re-Embedding name-only. Getestet auf echter PG: 38/38 (inkl. Anchoring-False-Positive-Guard, Multi-Typ-Person, Trailing-Dot).
- **Fix: Bridge-Resolver-Konflation** — `resolve_entity` bekommt `use_embedding` (Default True). Der Memory→KG-Bridge-Pfad (save-time + Backfill) nutzt jetzt `use_embedding=False`: bloße Vornamen ("Erika") embedden nah an andere gleich-tier Personen ("Anna Johanna von den Muster") und wurden sonst in die FALSCHE Person gefaltet (die ursprüngliche Konflation, auf Resolver-Ebene). Bridge resolved nun exact/surface → sonst NEU anlegen; echte Near-Dupes bleiben dem review-gated Reconciler. Verifiziert: use_embedding=False legt eigene Erika-Entität an.
...
### Behoben
- **n8n zeigte ins öffentliche Internet statt ins LAN (#1235)** — `N8N_API_URL` stand seit dem ersten Cluster-Deploy auf `n8n.home.example.dev` (Wildcard-A-Record-Rest beim Hoster → `*.web.hostpoint.ch`); der TLS-Handschlag scheiterte, MCP-Status meldete trotzdem grün. Umgestellt auf die LAN-Adresse. Aufgedeckt von der A1-Sonde am ersten Tag.
- **Folgefragen-Chips zeigten rohes JSON (#1236)** — bei einem JSON-Objekt statt Array wurde der Blob zu einem anklickbaren Chip; `_parse_followups` wertet jetzt Objekte aus und weist JSON-Blobs ab.
...
- **Rundansage an alle belegten Räume** (`internal.broadcast_announcement`). Eine öffentliche TTS-Durchsage, die in JEDEM Raum abgespielt wird, in dem gerade jemand anwesend ist — „Ansage an alle: Mittagessen", „sag allen Bescheid", „ruf alle zum Essen", „tell everyone …". Ergänzt die bestehende gezielte Nachricht (`announce_in_room`); gleiche `presence`-Rolle, der Agent unterscheidet per Tool-Beschreibung + Prompt, nicht per Routing. Geteilter `_announce_core` öffnet seine EIGENE DB-Session (eine `AsyncSession` ist nicht concurrency-safe); Einzel-Ansage synthetisiert lazy NACH dem Privacy-Gate, Broadcast synthetisiert EINMAL und fächert semaphore-begrenzt (Cap 4) auf. Nur öffentlich: `privacy='personal'` wird an der Grenze abgelehnt (kein N-faches Vision-Gewitter); dedupliziert belegte Räume per `room_id`; löst kanonische Namen aus der `room_id` auf (Presence-`room_name` ist nullable); ehrliche Pro-Raum-Zusammenfassung (Presence sieht nur BLE-getrackte Geräte → behauptet nie volle Abdeckung). Nebenbei behoben: der Roh-Lautsprecher-Fallback spielte nur auf dem ERSTEN Lautsprecher eines Raums — jetzt auf allen. PR [#851](https://github.com/ebongard/renfield/pull/851). Design: `docs/MESSAGE_RELAY.md`.
- **Berechtigungs-Gate für Durchsagen** (`HA_CONTROL`, fail-closed). Beide Ansage-Tools (`announce_in_room` + `broadcast_announcement`) leiten TTS auf Raum-Lautsprecher — eine hausweite Steuer-Aktion — und sind nun in `ha_glue_execute_tool` (`_HA_CONTROL_GATED_TOOLS`) auf dieselbe `HA_CONTROL`-Berechtigung wie `device_action` gegated. `user_permissions=None` (Auth aus ODER unidentifizierter Sprach-Turn ohne JWT / mit geringer Sprecher-Konfidenz) ist erlaubt — physische Sprach-Präsenz ist ihre eigene Autorisierung, „Ansage an alle" funktioniert weiter; bei vorhandener Berechtigungsliste (authentifizierter Nutzer) ist `HA_CONTROL` Pflicht, sonst Ablehnung VOR dem Tool-Body. Ein `Gast` (`ha.read`) kann damit keine raum- oder hausweite TTS auslösen, `Familie`/`Admin` (`ha.full` ⊇ `ha.control`) schon. Wiederverwendung von `HA_CONTROL` = keine Migration. PR [#852](https://github.com/ebongard/renfield/pull/852).
