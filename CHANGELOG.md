...
- **KG-Reconciler Person-Guard** (macht `KG_RECONCILER_ENABLED` sicher aktivierbar). Der Reconciler-Self-Join betraf bisher alle Entitaeten inkl. Personen; da distinkte Personennamen intrinsisch ≥ Kandidaten-Schwelle embedden (Jutta~Anna 0.894), konnte ein Personen-Paar bei ≥ Auto-Merge-Schwelle (0.95) ZWEI VERSCHIEDENE Personen still zusammenfuehren. Fix (spiegelt resolve's Personen-Embedding-Skip): ein Personen-beteiligtes Paar (primaer ODER Multi-Typ) mit UNVERWANDTEN Namen wird komplett verworfen — kein Merge, kein Proposal. Personen reconcilen nur bei VERWANDTEN Namen (gleich oder Token-Teilmenge: "Alice" ⊆ "Alice B.", "Jutta" ⊆ "Jutta van den Bongard"). Das Auto-Merge-Gate verlangt die Namensverwandtschaft fuer Personen zusaetzlich (Defense-in-Depth hinter dem Find-Drop), sodass eine Erkennungsluecke nie still zwei Personen verschmilzt. Nicht-Personen unveraendert. 19/19 PG-Tests (inkl. Vulnerability-Test + Gate-Bypass-Test).
...
### Behoben
- **n8n zeigte ins öffentliche Internet statt ins LAN (#1235)** — `N8N_API_URL` stand seit dem ersten Cluster-Deploy auf `n8n.home.bongard.dev` (Wildcard-A-Record-Rest beim Hoster → `*.web.hostpoint.ch`); der TLS-Handschlag scheiterte, MCP-Status meldete trotzdem grün. Umgestellt auf die LAN-Adresse. Aufgedeckt von der A1-Sonde am ersten Tag.
