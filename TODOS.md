...
- **Frontend:** ~~W9 React.lazy code-splitting~~ · ~~W11 Prettier~~ · **E11 React Query** (foundation + 5 reference pages landed in #504 on 2026-04-30; 19 pages + 7 components + 25 spec migrations + `useChatSessions` rewrite remain — mechanical follow-up using committed canonical patterns as templates) · ~~E12 13 hardcoded German strings~~ · ~~E13 ChatPage prop drilling → Context~~ (verified done — ChatInput takes 0 props, all from ChatContext) · ~~E14 ESLint React version~~ · **E15 enable `tsconfig` strict mode** (~31 errors, mostly null-checks — deferred to dedicated session) · W10 closed via #487 on 2026-04-27
