...
**2026-09-02 (actionable-backlog batch, Table-1):** Four tractable issues implemented in-cycle (reviewed + tested on .159), merged, deployed. **#448** (extract the triplicated `_resolve_owner_user_id` → `AtomOwnerResolverMixin`, `services/atom_owner.py`) + **#449** (`ATOM_TYPE_*` constants at every writer/emit call site) — pure refactor, PR #1195, 32 tests green. **#874** (route the agent's STRING KG-context path `KGRetrieval.get_relevant_context` through the shipped `expand_fused` seam → 1-2-hop multi-hop expansion for `internal.knowledge_search` + the ReAct loop; NO signature change, flag-off byte-identical, tier-safe per-hop + a tier-leak-guard test) — PR #1196, 4 new + 153 regression green on real PG; live since `GRAPH_EXPANSION_ENABLED` is already on. **#447** (durable real-Postgres integration tests for the `pc20260423_atoms_per_document` migration — gate-blocks-heterogeneous-tiers / MIN-collapse / upgrade→downgrade→upgrade cycle / per-document share-rowcount / 3-phase atom write; throwaway-schema harness binding alembic `op` to a `MigrationContext`) — PR #1197, 5 tests green, test-only. **Deliberately NOT implemented** (documented, not a shortcut): **#1116** login-audit residuals stay a tracker — all deferred/blocked (scope-enforcement A would break the not-yet-migrated Voice-WS; B is risky ingress infra; C/D are dead code until the Reva callback / LDAP land); and the Phase-5/epic items **#342** (4-week platform↔ha-glue extraction, Q3-2026 OSS), **#876** (cross-user/household KG canonicalization — blocked on named-circles v2, the shared-tier access primitive; a cross-user merge must never raise visibility), **#875** (bi-temporal `valid_at`/`invalid_at` edges — the issue itself requires an explicit go-ahead + its own design review), **#877** (`kg_entities.external_id` offline linker — needs a Wikidata/GND pipeline; column-only today), **#1121** (F6 Redis-auth/NetworkPolicies in the reva cluster — separate infra/repo; **closed 2026-09-04**). **#342 update 2026-09-20:** W1.2/W2/W4.2 are on main (PRs #343–#357 moved models/config/intent/lifecycle + 28 services/routes into `src/backend/ha_glue/`, #359 added `tests/backend/test_ha_glue_boundary.py`); open: W3 (platform-only Alembic baseline) + the W4.1 full run. PRs #1195–#1197.

**2026-09-02:** **Backlog batch — 8 GitHub issues closed** (triaged from the full docs+issues sweep). **Verified-shipped + closed:** #1137 (Scheduled Tasks subsystem) and #658 (Paperless async-finalize reconciler) — already live, formally closed. **Implemented + reviewed + tested (.159) + deployed on BOTH instances:** #668 (force-play on a busy **DLNA/output-provider** renderer — the busy/force guard read only `entity_id`, so `force=true` on a busy DLNA/provider device was a silent no-op; now accepts `dlna_renderer_name`/`output_target_id`); #450 (review-label chunk preview via Postgres **`DISTINCT ON`** instead of fetching every chunk of every reviewed doc; sqlite falls back); #446 (**`reap_orphan_placeholder_atoms`** + a `placeholder_atom_reaper` scheduled-task built-in — deletes orphaned `__pending__` atoms left by a crash mid `create_with_source`, age-floored so in-flight creates are safe; escaped-LIKE hardening from review); #881 (**folder-ingest renames the `processed/` file to the generated title** via a new filesystem-MCP `rename_processed` tool + backend hook, best-effort/idempotent/collision-/traversal-safe; `FOLDER_INGEST_RENAME_PROCESSED_ENABLED`, MCP `v0.1.8`); #1162 (**SearXNG functional health probe** — distinct contributing *general* engines + `unresponsive_engines`, `degraded`-not-down; the backbone set was made **ConfigMap-tunable** — `SEARCH_FUNCTIONAL_BACKBONE_ENGINES=bing,google-cse` — after catching an over-sensitivity where the CAPTCHA-prone `duckduckgo` scraper falsely flagged a healthy instance); #1161 (**forked `@kevinwatt/mcp-server-searxng` → `@x-idra/mcp-server-searxng`** carrying the unreleased upstream 0.3.11 fix: a reachable SearXNG instance returning 0 results is a valid empty search, **not** "All SearXNG instances failed"; npm's latest 0.3.9 still mislabels it. Vendored into the backend image; standalone fork `github.com/ebongard/renfield-mcp-searxng`). Each PR reviewed clean by a dedicated agent + tests green on .159. Also **flipped dark flags on both instances:** `NOTES`, `ROOM_HANDOFF`, `PAPERLESS_DEDUPE_RECONCILER`, `FOLDER_INGEST_RENAME_PROCESSED`, `SEARCH_FUNCTIONAL_PROBE`. En route: pinned the filesystem-MCP `mcp<2` (a rebuild pulled the FastMCP-renamed 2.x). PRs #1188–#1193. **Still open:** the #1170 P2 text-signal follow-ups below; nothing new deferred.

**2026-08-31 (release v2.24.0):** **KB near-duplicate DOCUMENT detection** shipped end-to-end (#1170 P1 #1172 / P2 #1173 / P3 #1175) — finds documents byte-hash ingest dedup can't (different bytes, same doc) and Paperless dedupe never sees. **Identifier signal** (shared `document_facts` `(kind, normalized_value)`, recurring-identifier frequency gate) + **fuzzy text signal** (`documents.content_embedding` = mean chunk embedding, index-accelerated HNSW top-k via `CROSS JOIN LATERAL` — the O(N²)-cross-join → O(N·k·log N) rewrite was the #1175 review catch) + **autonomous daily scan** (scheduled-task built-in). **Propose-only** → `/brain/review` `DocumentDuplicatesSection` (survivor + per-pair **supersede**/delete + 5s undo). **ENABLED + per-corpus-tuned on BOTH instances** (#1176): identifier gate `max_docs=2`, text @ cosine **0.995** (validated read-only first — the mean-embedding floods a homogeneous invoice corpus; xidra 0.97→163 vs 0.995→11 pairs). Migrations `pc20260830`/`pc20260901`, service `services/document_dedupe_service.py`. Also in v2.24.0: **Paperless filing tools** (#1171 — `internal.list_unfiled_documents` + `internal.refile_to_paperless`) and checksum-linked `paperless_document_id` (#1166/#1169); **`/wissen/dokumente` sortable** (name/import/`document_date`) + **Paperless/Simba status icons** (#1174, migration `pc20260831`); the **Simba tax-portal** bridge + watch-folder review queue + booking-period (#1149–1168); **ranked hybrid document search** (#1156); **SearXNG** retune/pin/dedicated-xidra (#1158–1160); DB pool txn-leak fix (#1157). Design: issue #1170 + CLAUDE.md dedupe/filing entries. **Follow-ups → P2 below** (text-signal false-positive tuning; max_docs=2 triplicate trade-off; PR2 federated document search).

**2026-08-28 (release v2.23.0):** Chat-agent reminders shipped (`internal.create_reminder`, #1146/#1147) — "erinnere mich in X…" now persists a real reminder (was hallucinated: `ReminderService.create_reminder` had no chat path, only `POST /api/reminders`). Tool wired into `agent_tools` + `action_executor` (user_id/session_id injected), routed via the `general` role (reminder clause added to `agent_roles.yaml`), H4 in-handler gate on `proactive_reminders_enabled`; `parse_duration` root-fix (naive wall-clock = LOCAL tz → naive-UTC; tz-aware ISO respected). **Live + E2E-verified on BOTH instances** (`PROACTIVE_REMINDERS_ENABLED=true` on household + xidra; browser chat → role "Allgemein" → reminder persisted). Also in v2.23.0: the **Scheduled Tasks subsystem** (#1137 — DB-defined interval/cron jobs, "Geplante Aufgaben" admin UI + per-run history, 21 built-ins incl. autonomous Paperless dedupe reconciler), **CNPG HA Postgres** (#1130-#1133), **HttpOnly-cookie auth + CSRF + login-hardening Tier-2** (#1116/#1125/#1128), and the **Paperless re-ingest-loop / index-independent dedupe / async-finalize** fixes (#658/#1124/#1134/#1135/#1136). Design: `docs/design/scheduled-tasks.md`.
...
| Sprecher-Phase-3-Flip | Kein Code offen: einlernen, kalibrieren — das Flag selbst steht im Haushalts-ConfigMap bereits auf `true` (`k8s/configmap.yaml:463`, #977) | `:285-288` |
...
| ~~`REQUIRE_EMAIL_VERIFICATION`~~ | ✅ entfernt 2026-09-20 (Flag ohne Funktion; mit `TRAJECTORY_REDACT_PII`, `MEETING_AUTO_MATCH_ENABLED`, `ARTIFACTS_HTML_SANDBOX_ENABLED`, `WAKE_WORD_ENABLED`) | — |
...
~~Letzter-Admin-Schutz TOCTOU-anfällig~~ (behoben in #1122: `acquire_last_admin_guard_lock`, `pg_advisory_xact_lock`) · Föderations-`remote_user_id`-Kollision ab dem 2. Peer (`:100`).
...
### S6 — Aufräumen
~~#83 schließen~~ (geschlossen 2026-09-18) · ~~#12 als Dublette zu #80 schließen~~ (geschlossen 2026-09-18) ·
~~#1218 und #1215 auf den Rest zuschneiden~~ (Phasenkästchen nachgezogen 2026-09-18; offen je Phase 3) · #270/#264 prüfen ·
...
~~OCR-Evaluation nie gelaufen~~ (lief 2026-07-23 über 148 Dokumente; Tesseract ist seit #1033 Default — `docs/design/ocr-engine-eval.md` §Outcome) ·
...
- ✅ **~~🔐 Full security audit — login & user management~~ PERFORMED** — first pass 2026-07-21 (remediation PR #1014 merged), second pass 2026-08-23 (#1116, remediation batches #1122/#1123). Residual findings are tracked in **#1116**; scope record in **P2 → "🔐 Full security audit — login & user management"**.
...
### 🎙️ Browser voice on xidra — follow-ups (code + infra prepared on `feat/xidra-browser-voice`, flag NOT flipped)
...
- **Text-signal precision.** The mean-chunk-embedding signal conflates "same layout/vendor" with "same document" on a homogeneous corpus — mitigated today only by a high threshold (0.995) + propose-only. Better: a **blocking key** (same issuer/date/page_count) before the cosine, or a sharper doc signature than mean-pooling. Only worth it if 0.995 proves too noisy (watch the reject rate on `/brain/review`). Primary: `services/document_dedupe_service.py::find_text_similar_pairs`.
- **`max_docs=2` triplicate trade-off.** Both instances run the identifier gate at `recurring_identifier_max_docs=2` (tuned down from 3 to cut recurring-reference false positives 174→79 / 168→89). This **misses a document genuinely present in exactly 3 copies** (its identifier sits on 3 docs → excluded). Acceptable for now; revisit if a real 3× duplicate is reported. Flag: `DOCUMENT_DEDUPE_RECURRING_IDENTIFIER_MAX_DOCS`.
- **content_embedding backfill on new/other instances.** Enabling the text signal anywhere new requires `bin/backfill_document_content_embeddings.py --commit` first (reuses chunk embeddings, no re-embed) + a read-only threshold sweep before flipping the flag. New ingests self-populate.
- **PR2 — federated document search** across Paperless + (xidra) Simba, deferred from the ranked-hybrid-search work (#1156). Primary: `services/document_search.py`.
...
- **✅ DONE — committed WER gate** (`wer_max: 0.40` in `tests/eval/diarization/gates.yaml` + `_wer_gate` in `bin/run_diarization_eval.py`): a hard-when-measured regression gate that BLOCKS the gross-ASR-failure class (the German-on-English ~0.95-WER bug would fail) while clearing good runs; advisory-n/a when `jiwer` isn't installed so a WER-less run never false-fails. **✅ DONE — bigger-model eval** (large-v3-turbo, see above) and a **✅ German ASR floor** (2026-07-22): **5.3% WER** on FLEURS `de_de` read speech (30 clips / 681 words, `language=de`, resident model) — near-perfect (errors like "hong kong"/"hongkong"). Confirms the German engine is STRONG; the meeting WER gap (English AMI was 27.5% ASR / 34% end-to-end) is driven by the meeting REGIME (spontaneous + overlap), NOT language, so German *meetings* should land in a similar range and large-v3-turbo pushes it lower. A representative German *meeting* number still wants a real xidra German meeting with a human-corrected reference (read-speech is a floor). Eval: `bin`-less `german_wer.py` on the household PVC (FLEURS via `datasets`, `Audio(decode=False)`+soundfile to dodge the torchcodec dep).

### 🗂️ Meetings UX — surface the VALUE, don't bury it (operator feedback 2026-07-21)
The whole point of a Besprechung — the minutes (summary/decisions/action-items) — was hidden **inside an expanded meeting card on `/meetings`, below the full transcript**. Nobody scrolls a 100-line transcript to find it, so generated minutes sat **unconfirmed and invisible** (verified: xidra meeting #7 has a `draft` nobody has seen). Operator's stated blocker: *"the UX is bad."*
- **PHASE 0 SHIPPED** (`feat/meeting-phase0`): a **"Protokoll: Entwurf bereit" badge + jump** on the collapsed card (`minutes_status` now on `GET /api/meetings`) and the expanded card inverted to **minutes above a collapsible transcript**. Plus the KG **pseudonym strip** (`Sprecher N` no longer leaks as a graph entity). Design `docs/design/meeting-kg-and-speaker-identity.md` (Track D).
- ✅ **DEDICATED DETAIL PAGE SHIPPED** (`feat/meeting-detail-page-track-d`, 2026-07-23): `pages/MeetingDetailPage.tsx` at `/meetings/{id}` — minutes/decisions/action-items are the **default view up top**, the raw transcript is secondary + **collapsed** below, and a prominent **draft-confirm nudge banner** keeps generated drafts from rotting. Completed list cards are now **links** to the detail page (inline expand removed — the card affordance was the first slice). Shared building blocks extracted to `components/meetings/` (`StatusBadge`/`TranscriptView`/`MinutesPanel`/`ProjectSelect`), `useMeeting(id)` → `GET /api/meetings/{id}`. de/en i18n; MeetingsPage tests rewritten + new `MeetingDetailPage.test.tsx`.
- ✅ **BIDIRECTIONAL DEEP-LINK SHIPPED** (2026-08-25): the project timeline's meeting/decision events now deep-link to the **meeting detail page** `/meetings/{id}` (was `/meetings` list, and meeting_id now wins over the transcript's document_id so it lands on the deliverable-first view, not the raw KB transcript); and the meeting detail page shows an **"Open project" link** → `/projects/{id}` when linked (`ProjectDetailPage.tsx` href reorder + `MeetingDetailPage.tsx` link + de/en i18n; RTL assertions both directions).
- **REMAINING (Track D follow-up):** auto-match / stronger confirm reminders are separate tracks.
- **RELATES TO:** the meeting→KG redesign — the same "structured output is the product, raw transcript is the source" principle. The meeting↔project link (`feat/meeting-project-linking`) should land with this UX, not the buried-in-a-card one.

### 🔐 Full security audit — login & user management (PERFORMED 2026-07-21, remediation PR #1014 merged; second pass 2026-08-23 → residuals tracked in #1116)
**STATUS (2026-07-21):** adversarial audit run (4 independent subagents over authn/login · token lifecycle · authz/RPBAC/IDOR · WS/CSRF/frontend, OWASP-ASVS-grounded), findings verified against source. **Remediation branch `security/auth-audit-remediation`** (backend 29 new tests + 97 regression green on `.159`) fixes: **H1** roles route grant-only-what-you-hold (a `roles.manage` non-admin could add `admin` to a role → takeover), **H2** users route grant-only + block self-role-change (a `users.manage` non-admin could self-assign the Admin role), **H3** `token_epoch` session revocation on password change (was: change didn't invalidate live tokens), **H4** logout revokes the refresh token too + returns 503 on blacklist-write failure (**M6**), **M3** login timing-equalization (bcrypt runs on the not-found branch — no username-enumeration oracle), **M5** last-admin lockout guards (demote/deactivate/delete), **M2-backend** short-lived `scope:"ws"` WS token endpoint + REST rejection (groundwork). **Deferred (own follow-ups, each with a reason):** **M1** — delete the `#access_token=` fragment handler: it is the LIVE consumer of the cross-repo Reva OIDC callback's success redirect, so a standalone delete would break federated login; its safe removal = REPLACE it with the PKCE `?code=` flow as part of the **SSO cutover** below (audit confirmed it as a session-fixation sink; risk is unchanged from today). ~~**M2-frontend** — browser WS clients fetch the short-lived token~~ ✅ SHIPPED (#1082: `utils/wsToken.ts` `fetchWsToken()` wired into `useChatWebSocket` + `useKioskSocket` (#1082) and `useUserEvents` from its creation (#1183); voice-WS via `fetchVoiceToken()`, #1128). HttpOnly-cookie migration (JWT item below). Low/Info items: ~~open-registration default~~ ✅ hard boot gate for auth-on prod/staging with an inherited default (2026-09-20, BL-0124), ~~username-keyed lockout DoS~~ ✅ lockout scoped per (username, IP) with a username-wide backstop + admin unlock `POST /api/users/{id}/unlock` (2026-09-20, BL-0125); ~~unauth taxonomy/MCP enumeration on `/api/roles/permissions/all` + `/api/auth/permissions`~~ ✅ gated on `ROLES_VIEW` (#1122); ~~WS Origin check~~ ✅ `_ws_origin_allowed` (#1123). **⚠️ MERGE/DEPLOY GATE — verify Reva (federated OIDC) login still works** (operator to check, 2026-07-21): the remediation PR #1014 adds the `token_epoch` claim to `get_current_user`. Reva-minted tokens have NO epoch claim → read as 0 → accepted for `token_epoch=0` users (the normal SSO case; backward-compat is unit-tested), and the `#access_token=` fragment hand-off is untouched (M1 deferred) — so no regression is EXPECTED. But it is not runtime-confirmed (the Reva submodule isn't in the renfield checkout; needs the live OIDC dance). Action: (a) confirm whether Reva OIDC is even enabled on xidra (`VITE_OIDC_ENABLED` / the Reva callback); if enabled, (b) post-deploy **browser E2E of the actual Reva sign-in**, and (c) one Reva-repo check that its callback still mints via renfield's `create_access_token` (old signature is fine — the new `token_epoch` param is optional, no contract bump needed). Edge risk to watch: a Reva-SSO user whose `token_epoch` got bumped >0 (only via change-password/admin-reset, which SSO users don't hit) would be rejected. ~~**P2 from the /review pass: the M5 last-admin guards are TOCTOU-racy**~~ ✅ FIXED in #1122 — `auth_service.acquire_last_admin_guard_lock` (`pg_advisory_xact_lock`) serializes the guarded mutations in `routes/users.py` + `routes/roles.py`; tests in `test_auth_audit_remediation.py`. Full findings + fixes in the PR.
**Trigger:** the operator wanted a **full security audit of the login and user-management functionality** once the Notes + project-timeline (business-instance Phase 4) work is finished. It is the umbrella over the two auth debts below (SSO hand-off cutover, JWT/session storage), plus a fresh sweep, not just those.
- **SCOPE:** the whole authn/authz surface — `api/routes/auth.py` (login / refresh / register / change-password / sso-exchange / voice-auth / me), `auth/login_flow.py` + the pluggable provider registry (`auth/registry.py`, `db`/`ldap`/`google`/`github`/`apple`), `services/auth_service.py` (JWT mint/verify, `get_current_user`, blacklist), `services/login_lockout.py`, `services/token_blacklist.py`, the WS `?token=` handshake (`main.py` `/api/ws/token`), RPBAC (`models/permissions.py`, `ProtectedRoute`/permission gates), user CRUD (`api/routes/users.py`, roles), password policy + reset flow, `must_change_password` enforcement, and the account-lifecycle (create/disable/delete, speaker-link `users.speaker_id`).
- **WHY:** xidra is the first real **auth-on multi-user** instance (household is auth-off), so the login/user-management path now carries real trust boundaries that the household deploy never exercised. Before the auth-on cutover widens (more users, federation identity mapping, SSO enablement) the surface should get one dedicated adversarial pass.
- **HOW:** an adversarial `/review`-grade pass (consider a fresh independent-subagent audit like the SSO PR got) against OWASP ASVS auth chapters — enumeration oracles, lockout bypass, token lifetime/rotation/revocation, privilege escalation across roles, IDOR on user routes, CSRF on cookie/redirect paths, secret handling (`SECRET_KEY`↔JWT↔IRK coupling), and the provider-registry fail-open walk.
- **DEPENDS ON:** Notes + project-timeline done (the stated trigger). Related open auth tracks that should fold in: the SSO cutover + JWT/session items below, `docs/private/security/*` (satellite-trust H1/H6), and the **auth-on cutover** for the household (see deploy-production "Standing up a NEW instance (auth-on)").
- **FOLD IN — ungated agent Paperless write (found 2026-07-26).** `mcp.paperless.update_document` is agent-visible in the `documents` role (`mcp_servers.yaml` `prompt_tools`) with **no permission gate** beyond role membership — any user routed to that role can rewrite Paperless metadata (title/correspondent/`created`/tags). Harmless under auth-off / single-user, a real authz gap once auth-on multi-user widens. Fix options: gate it behind a permission (e.g. `RAG_MANAGE`, like `internal.reextract_paperless_metadata`) by routing writes through a gated `internal.*` wrapper and removing `update_document` from `prompt_tools` (as `upload_document`/`delete_document` already are), OR accept it as a documents-role capability. Deliberately deferred here (operator call 2026-07-26): the ingest date-drift fix stops new drift; this is a separable hardening.

### SSO token hand-off — complete the cutover (fragment handler removal + emitter migration)
Origin: `feat/sso-token-handoff-hardening` (#1004, merged + deployed **dark** to xidra `2026-07-20-sso`, 2026-07-20). The one-time-code + PKCE **receiver** is shipped (`services/sso_handoff_store.py`, `POST /api/auth/sso/exchange`, `pages/AuthCallback.tsx` + `/auth/callback`, `utils/pkce.ts`; `SSO_HANDOFF_ENABLED` dark). Design + threat model + rollout: [`docs/design/sso-token-handoff-hardening.md`](docs/design/sso-token-handoff-hardening.md).
- **WHAT REMAINS (the actual vuln closure):** the legacy URL-fragment token handler (`main.tsx::_consumeOidcHashHandoff`, `#access_token=` — implicit flow + token-injection/session-fixation sink) is **still present** (the 2026-07-21 audit CONFIRMED it as a session-fixation sink — M1 — but a standalone delete was rejected: it is the live consumer of this Reva OIDC callback's success redirect, so deleting it before the emitter moves to `?code=` breaks federated login). Cutover = (1) migrate every **emitter** to redirect with `?code=` instead of `#access_token=` — the emitter today is the **cross-repo Reva OIDC callback**; (2) enable `SSO_HANDOFF_ENABLED`; (3) delete the fragment handler (safe only once 1+2 are done — this is audit M1's safe form). Also **un-wired**: `GET /api/auth/sso/start` + the "Sign in with…" PKCE initiation (Renfield's own `google`/`github`/`apple` redirect providers are `enabled=False`).
- **BLOCKING OPEN QUESTION (§10.1):** where the Reva OIDC callback runs relative to Renfield's Redis — decides whether the emitter writes the handoff code store directly, or redirects to a Renfield `…/sso/finish` endpoint that does the store+redirect. Needs the operator's call before the emitter change.
- **WHY:** until cutover the fix ships inert code; the token-in-URL exposure (history/extensions/injection-fixation) stays open on any instance running a federated-login redirect. **DEPENDS ON:** the Reva-side change (cross-repo) + §10.1.

### ~~JWT / session storage — the deferred HttpOnly-cookie hardening~~ ✅ SHIPPED + LIVE (#1125 + #1128, 2026-08-25)
The session now lives in HttpOnly+Secure+SameSite=Lax cookies + stateless double-submit CSRF (`AUTH_COOKIE_ENABLED`, flipped on xidra). Cookie-first-then-Bearer dual-read kept Reva/voice/legacy working. The **voice-WS follow-up (#1128)** replaced the last localStorage JWT (the voice `?token=`) with a short-lived `scope:"voice"` faucet token, so in cookie mode NEITHER access nor refresh token is JS-readable — the XSS token-theft surface is closed on the business instance. `SECRET_KEY` untouched (tri-purpose). Deployed both instances (tags `2026-08-25-cookie-auth` / `-voice-ws`), E2E green. **Remaining debt (own items):** the SSO cutover below (Reva `?code=`); retiring the `/api/ws/token` faucet + Bearer interceptor once Reva is cookie-native; enabling browser voice on an auth-on instance (then the full live `scope:voice` E2E). See CLAUDE.md §"JWT HttpOnly-cookie session + CSRF" + `docs/runbooks/cookie-auth-flag-flip-xidra.md`.

### Federation multi-peer: `remote_user_id` collision in the kept tier-membership arm
Origin: `/review` of PR #957 (federation responder circle-enforcement). The peer-scoped circle filter and `FederationQueryResponder._resolve_asker_tier` both key the tier-membership on `circle_memberships.member_user_id = peer.remote_user_id`. `remote_user_id` is the REMOTE instance's user id, and `member_user_id` is an FK to the LOCAL `users.id`. With **two** peers paired to one local owner, both numbering their users from 1, their memberships collide to the same integer: `_resolve_asker_tier`'s `scalar_one_or_none()` would raise `MultipleResultsFound`, and the SQL membership arm `cm.member_user_id = :asker` could match the WRONG peer's grant (a peer sees atoms granted to a different peer at that tier). **Not introduced by #957** — the fix REDUCES the collision class (it removed the owner + explicit-grant equality branches); this is the residual in the one arm that must stay. Only bites with ≥2 peers under one owner (single bidirectional links are safe).
- **WHAT:** Give each `PeerUser` a per-peer LOCAL identity namespace so `member_user_id` (and the responder's `asker_id`) is unique per peer, not the raw `remote_user_id`. Likely a dedicated local surrogate id minted at pairing (or composite `(peer_id, remote_user_id)` keying). Then `_resolve_asker_tier` and the membership arm are collision-free.
...
### Person-scoped federation (cross-instance identity mapping) — F-ID-1 SHIPPED dark (#959/#960); open: F-ID-2 handshake + activation
**Stand 2026-09-20:** F-ID-1 is on main — `federation_user_links` (migration `pc20260712`), `services/federation_identity_link.py`, ADMIN CRUD `api/routes/federation_user_links.py`, durable instance identity (#960); flag `federation_identity_links_enabled=False`. Still open: the consent-signed double-login handshake (F-ID-2) and activation (prereq below).
...
- **PREREQ (P0):** personal instance → auth-on (real per-person accounts). Big separate track — design: `docs/design/household-auth-on-cutover.md` (decided 2026-09-21, P0 posts in build) (see deploy-production "Standing up a NEW instance (auth-on)").
...
### ~~Graph-expansion relation-tier filtering for the non-federation (household/auth-on) path~~ ✅ SHIPPED (#1116 Tier-2 Batch A, #1122)
Origin: `/review` of PR #957. `_relation_filter` (`services/graph_expansion.py`) now mirrors `_entity_filter`'s `auth_enabled`-gated activation — relations are circle-tier-filtered on the auth-on household path too, not only the federation (`enforce_circles`) path. Closed the disclosure class where a household member seeing two entities could see a stricter-tiered relation between them. (Verified 2026-08-25; entry had gone stale.)
...
### ~~KB maintenance tools: distinguish genuinely-unindexable docs from transiently-failed 0-chunk docs~~ ✅ DONE
Shipped in `services/kb_maintenance_tool.py`: `_unindexable_exists()` classifies REPAIRABLE vs UNINDEXABLE (via `document_processing_history` — a completed run that produced 0 usable chunks AND dropped everything at the quality gate / was an already-retried re-derivation), `list_chunkless_documents` labels each, `reindex_documents` skips unindexable by default (`force=true` overrides) and filters in SQL so the cap applies to repairable docs, `ingest_status` splits `chunkless_reindexable`/`chunkless_unindexable`. This duplicated the (already-marked-DONE) P1 entry above; closed out in the 2026-07-23 sweep.
...
- ~~**[P2] Docling+poppler union extractor — blocks trustworthy sums.**~~ ✅ SHIPPED #637 — `document_processor.extract_text_layer` (pdftotext, poppler-utils in the backend image) is unioned with the Docling text as `===TEXT-LAYER (raw)===` for field extraction. Residual: the union is skipped when the text layer is judged unusable, so pure scans still depend on OCR/VLM alone.
...
- ~~**Multi-replica `pg_try_advisory_lock`.**~~ ✅ COVERED — the sweep runs only as the scheduled-task built-in `paperless_ui_edit_sweep` (sole caller of `run_sweep_tick`), and the task engine takes a per-task Postgres advisory lock per run (`scheduled_tasks/engine.py`), so it is single-flight across replicas. The top-of-file comment on `_sweep_lock` in `paperless_ui_edit_sweeper.py` still claims the DB lock is missing — stale, fix when next touching the file.
...
**✅ LARGELY RESOLVED 2026-07-09/10 — root cause was satellite-report cadence asymmetry, not the backend.** Live diagnostics (`GET /api/presence/debug/sightings` + per-satellite RSSI-over-time chart) showed the Esszimmer k8s pod reporting BLE every **3s** while the bare-metal Pis reported every **30s** (`ble.scan_interval` default). The satellite loop `sleep(scan_interval)` gates the *report* rate in BOTH periodic and continuous modes (continuous only steadies RSSI via EWMA — it does NOT lower cadence), so a 3s sat got 10 filtered-RSSI updates per 1 from a 30s neighbour → held/stole the room regardless of signal (the "phone in Wohnzimmer shows Esszimmer" bug + the RSSI-filter review's F1). **Fix = equalise cadence fleet-wide:** `group_vars/satellites.yml` `ble_scan_interval 30→3` + `ble_continuous true` (continuous makes the 3s poll near-free — in-memory snapshot, no extra radio bursts; proven on Pi Zero 2 W BT 4.2). Rolled to wohnzimmer/arbeitszimmer/fitnessraum/kinderbad (benszimmer offline). **Post-fix: distinct-room moves crisp, 18min stationary = zero bounces, mislocation gone.** Noise floor ~±4 dB vs real moves ~±20 dB; the RSSI filter's `margin=4` (env override) validated fine for real rooms — the only jitter was two sats placed **side-by-side for testing** (degenerate). Follow-ups: (a) re-test the Esszimmer↔Arbeitszimmer boundary once sats are back in real positions, maybe nudge margin to 5-6 for noise headroom; ~~(b) merge `feat/presence-debug-sightings` (endpoint + group_vars) after review~~ ✅ merged #942/#943 (`GET /api/presence/debug/sightings`). See [[project_presence_rssi_filter_landed_active]]. The RSSI-filter itself shipped in #941 (below).

**Historical context (pre-fix, #777, v2.17.15).** Media Follow Me now correctly follows the user *and* no longer jumps to empty rooms, but a genuine room change took **~1-2 min** before presence switches (and the music follows). Live-measured 2026-06-14: walked back to Arbeitszimmer, switch fired ~1-2 min later.
...
- ⬜ **Phase 3 flip (operational, blocked):** enroll ≥ 2 household members via `/speakers`, run `bin/calibrate_speaker_threshold.py`, set `speaker_recognition_threshold` + `speaker_match_min_margin` from its recommendation. ~~then flip `speaker_controlled_enrollment_enabled` on~~ — the flag is already `true` in the committed household ConfigMap (`k8s/configmap.yaml:463`, #977); enrollment/calibration state is not visible from the repo. No code left. With controlled enrollment on, the Phase-0 `speaker_quality_gating_enabled` gate is subsumed (`speaker_resolver.py`: `quality_active = gating or controlled`, profiles immutable).
...
- ~~**OCR engine evaluation / swap.**~~ ✅ DONE 2026-07-23 — the harness (`bin/run_ocr_engine_eval.py`, #1028; Tesseract baked into the image in #1029) ran over all 148 flagged documents; `docling_tesseract` won (drop ratio 0.68 → 0.30, 111 ↑ / 8 ↓) and is the default since #1033 (`rag_ocr_engine="tesseract"`, fail-safe to EasyOcr). Record: `docs/design/ocr-engine-eval.md` §Outcome. Original framing kept below. WHAT: benchmark the current OCR engine on the historical garbage docs; if an alternative reduces failure rate by >50%, plan the migration. WHY: root cause of the OCR quality problem is upstream engine quality; everything else (heuristic filter, re-OCR trigger, retrieval filter) is a layered workaround. **NOTE (2026-06-17 sweep): premise refreshed** — the engine is **already docling-based** (`document_processor.py`: docling `OcrAutoOptions`/`EasyOcrOptions`, `ocr_engine` ∈ `docling`/`docling_full_page_ocr`/`poppler_text_layer`), NOT the bare `easyocr` the old WHAT assumed, so the "easyocr→docling-default" comparison is moot; reframe as "docling-OCR vs tesseract/cloud" if pursued. **Cons:** offline-first ethos limits cloud options; days of benchmarking + integration. **Trigger:** scale-driven — defer until ≥hundreds of operator-flagged quality failures justify the cost.
...
### ~~Run `/design-consultation` to formalize DESIGN.md (BEFORE next major frontend surface)~~ ✅ DONE — `DESIGN.md` exists at the repo root (first landed with #401) and CLAUDE.md declares it the source of truth for every UI change. Original rationale kept below.
...
- ~~**Agent string path:** `get_relevant_context` does NOT yet go through the fused path~~ ✅ **ERLEDIGT via #874** (PR #1196, 2026-09-02). Gegengeprüft 2026-09-18: `services/kg_retrieval.py:391,:407` ruft `expand_fused` **innerhalb** von `get_relevant_context` (Zeilen 206-452); der Kommentar dort nennt ausdrücklich „the SAME expand_fused seam PolymorphicAtomStore uses, so the agent loop … not just the fused-store path". Der Eintrag war nicht nur veraltet, sondern behauptete das Gegenteil des Codes.
...
**WHY:** v3 KG-as-brain migration is currently "open-ended" because today's KG retrieval (`services/kg_retrieval.py`, with `graph_expansion.py` for the multi-hop step) is significantly weaker than chunk-level RAG. v2.5 closes that gap so v3 becomes a clean swap. v2.5 also unparks the broader retrieval-quality work in `docs/RAG_PARITY_PLAN.md` (which was parked pending real usage signal).
...
- 5 sub-items, ROI-ordered:
  - ~~**KG-1 multi-hop traversal** (~3 weeks)~~ ✅ SHIPPED as `services/graph_expansion.py::expand_fused` (level-synchronous BFS, hop decay, per-hop circle filter; `GRAPH_EXPANSION_ENABLED` on; agent string path via #874/#1196).
...
- Minimum-viable subset: KG-2 + KG-4 ≈ 4 weeks (KG-1 already shipped) for ~70% of practical benefit.
...
### ~~Notes Feature Design Doc (markdown editor + bidirectional links)~~ ✅ SHIPPED (Phases 4A/4B/4B.3)
**SHIPPED as a first-class `note` atom_type** — `notes` table (migration `pc20260720`), `services/note_service.py` (via `AtomService.create_with_source`, circle-tiered), hybrid dense+FTS retrieval (`services/note_retrieval.py`, `pc20260721`), `[[bidirectional links]]` on the KG substrate (`services/note_links.py`), REST CRUD `/api/notes`, `pages/NotesPage.tsx` + the `/wissen` Notizen lens, markdown/wikilink rendering (`NoteMarkdown.tsx`). Gated `NOTES_ENABLED`. Design: `docs/design/notes-atom.md`; see CLAUDE.md → Notes. The original design-doc-request rationale is kept below as history. Closed out in the 2026-07-23 sweep.
...
### ~~Re-enable `itsm` MCP on build-host when USU customer service is back~~ — entschieden 2026-09-20: kein Renfield-Posten (gehört ins Reva-Backlog — der Schalter liegt in der Reva-ConfigMap auf dem Reva-Build-Host; Renfield kennt `itsm` nur als Domänen-Schlüsselwort im Router und als `source_type` der Wissensbasis, beides unberührt)
...
PRD is unaffected (separate cluster, separate ConfigMap, `usu-mcp` actually running there). Build-host-only loose end.

- **Reverse with:** `ssh <user>@<reva-build-host> -- kubectl edit configmap reva-mcp-config -n reva` and flip `enabled: false → true` in the itsm block, then `kubectl rollout restart deployment/reva -n reva`.
