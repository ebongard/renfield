...
- **Bigger whisper model for meetings** — `meeting_whisper_model` already supports a per-job model off the resident STT slot (`rest_voice.py` loads it per job, freed after). Measure `large-v3` vs the resident model's WER/latency/VRAM on AMI; it's the single biggest lever and costs only GPU time per meeting (not live-STT contention).
