...
- **Cleanup script `bin/purge_low_quality_chunks.py` with the 4 correctness guards.** WHAT: scan `document_chunks` for `is_low_quality_text` matches, group by document, requeue affected docs via `RAGService.reindex_document(force_ocr=True)` so the new ingestion gate runs and the Document.atom_id stays stable. WHY: existing ~159-chunk corpus still has OCR garbage; today's retrieval-time filter is a workaround. **Correctness guards (must include):** (a) Alembic migration adds `ondelete='CASCADE'` to `DocumentChunk.parent_chunk_id` self-FK so cascade-delete via the parent Document doesn't trip NO ACTION FK violations. (b) `pg_try_advisory_lock(doc.id)` per document (or `status != 'processing'` check + skip) to avoid racing user-initiated reindexes. (c) Already-forced short-circuit hardcoded in the script too — don't re-trigger if the doc was already ingested with `force_ocr=True`. (d) Startup sweep in `lifecycle.py` that re-enqueues `status='processing' AND updated_at < now() - interval '1h'` so a script crash mid-batch doesn't leave zombie docs forever. Dry-run by default; `--apply` to execute. **Source:** Adversarial subagent findings 1, 3, 5 during the v2.10.4 /plan-eng-review.
...
### Brain quality — PR #625 follow-ups (multilingual FTS — out of PR scope)
PR #625 shipped multilingual FTS (DE/EN/FR/IT/ES/NL union) for `conversation_memories.search_vector` and made the alembic transaction model migration-friendly (`transaction_per_migration=True`). Two items deferred:

- **Multilingual FTS parity for `document_chunks.search_vector`.** WHAT: convert the chunk-side `search_vector` to a GENERATED-column union of `to_tsvector` across `services/fts_languages.FTS_LANGUAGES` (same approach as `pc20260528` for memories) so French/Italian/Spanish/Dutch documents stem correctly. Update `RAGRetrieval.search` and `LexicalRetrieval.search_chunks_lexical` to union `websearch_to_tsquery` across the same set; drop the per-call `RAG_HYBRID_FTS_CONFIG` parameter (or repurpose as a fallback for legacy single-language deployments). WHY: today a French PDF is stemmed with `'german'` (or whatever the deploy picked), losing recall on inflected words. **Cons:** `document_chunks` is a much larger corpus than `conversation_memories` (hundreds of thousands of rows on prod) — the GENERATED-column rebuild + GIN REINDEX is non-trivial (minutes-to-hours downtime window). Plan as a maintenance-window migration or use `CREATE INDEX CONCURRENTLY` + atomic-swap pattern. **Trigger:** before the first non-German-speaking household onboards documents, OR when an operator reports French/Italian/Spanish chunk search returning fewer hits than the same content in German.
- **Postgres integration test for the FTS branch of `search_memories_lexical`.** WHAT: add a Postgres-only test (mark `@pytest.mark.database` + skipif sqlite) that seeds memories in different languages and verifies cross-language matching works through the GENERATED column + `websearch_to_tsquery` union. Today the unit tests cover the sqlite LIKE branch only; the Postgres branch is exercised by the prod smoke but not by CI/regression. WHY: the multilingual union is the load-bearing piece; any change to `FTS_LANGUAGES` or the SQL builders should be caught by a test before deploy. **Source:** Adversarial subagent finding #1 during the PR #625 /review (judged INFO-tier, accepted as follow-up rather than block).
