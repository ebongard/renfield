...
- **REMAINING (Track D follow-up):** a **dedicated meeting detail page** (`/meetings/{id}`) with minutes/decisions/action-items as the default view (not the raw dialogue), and a stronger confirm nudge so drafts don't rot. The list-card affordances are the first slice, not the whole thing.
