...
- ~~**Frontend / GUI to surface facts — the three layered surfaces.**~~ **ALL THREE SHIPPED** (`/brain` in #643; the two dedicated surfaces built on `feat/schicht-a-gui`, eng + design + `/review` cleared 2026-06-01).
  ...
  Delivered cross-cutting: shared `FactProvenance` (`✓` deterministic / `~` advisory + low-confidence hint) + `ObligationRow` + `TierBadge` on every fact; `.fact-group` / `.legal-flag` / `.toast` / `.atom-row--bestaetigt` tokens (DESIGN.md-compliant, `--color-cream`, dark + reduced-motion); full de/en i18n. **Future feature — proactive calendar auto-push (Calendar MCP).** The Calendar MCP (`renfield_mcp_calendar`) has a write-capable `create_event` (per-user `visibility`/`owner_id`, CalDAV/Google/EWS), gated by `CALENDAR_ENABLED`. The agent can already create events on request today. A *proactive* auto-push (every obligation → a synced calendar event) is a notifier-sized feature, NOT polish: needs scheduler→`mcp_manager` wiring, an `(obligation, calendar_event_id)` **sync ledger** (avoid dup/stale events across daily/weekly scans), a target-calendar choice (work/family/per-user default), and lifecycle (confirm/reopen/date-change → update/delete the event). Deferred per user decision (2026-06-06); the portable `.ics` export ships now as the lightweight path.

**Still deferred:** the `Fakten` filter chip on `/brain`; `.ics` export. ~~per-fact inline `TierPicker`~~ **SHIPPED 2026-06-06** as a real per-fact tier *override* (`document_facts.tier_overridden`, sticky both ways, reset route + drawer UI) — not the naive "tier follows the doc" version; see the 2026-06-06 header note. **P2 follow-up:** preserve per-fact overrides across re-ingest/re-OCR (currently a re-extraction recreates facts fresh and resets overrides to the doc tier).
