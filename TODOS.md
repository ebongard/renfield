...
**Real per-fact fix (P2, NOT done):** subsume only when THIS turn actually captured a relation for the subject — requires **coordinating the currently-uncoordinated memory-extract and KG-extract async tasks** (e.g. run KG extraction first / inline and pass its captured-relations set into the subsume decision, or move both behind one orchestrated extraction with a shared result). Until then the guard is harm-reduction, not a complete fix. **The guard does NOT make `MEMORY_SUBSUME_TO_KG` safe for multi-user** — cross-user subject resolution + tier reach are unaddressed; keep subsume single-user only.
