...
- **REMAINING (Track D follow-up):** the meeting↔project link (`feat/meeting-project-linking`) UX could deep-link into this detail page; auto-match / stronger confirm reminders are separate tracks.
...
### JWT / session storage — the deferred HttpOnly-cookie hardening
Origin: SSO review (#1004) §9 + the session's auth work. The app stores the **access + 30-day refresh JWT in `localStorage`** (`utils/authTokens.ts`, read by axios + the WS handshake + kiosk), so **any XSS steals a 30-day refresh token** (revocable only via the blacklist / `SECRET_KEY` rotation). The SSO PR deliberately kept Bearer/localStorage to stay minimal.
- **WHAT:** evaluate moving the session to an **HttpOnly + Secure + SameSite cookie** so the token is never reachable from JS at all (immune to XSS token theft). The SSO `exchange` endpoint is the natural first place to set a cookie instead of returning a body (§9 notes it doesn't block this). Requires: convert the axios `Authorization: Bearer` interceptor + the WS `?token=` handshake + kiosk auth to cookie auth, and add **CSRF protection** on every mutating route (the reason it wasn't done inline).
- **RELATED JWT debt:** refresh-token delivery (body vs cookie — SSO design §10.2); refresh lifetime (30d) vs rotation-with-reuse-detection (already in `/refresh`, #698); the `SECRET_KEY`↔JWT↔IRK coupling (rotating `SECRET_KEY` invalidates all sessions AND destroys encrypted IRKs — documented, but a single-blast-radius secret); token blacklist fail-closed behavior under Redis outage.
- **WHY:** biggest remaining token-theft surface once the URL-fragment path is closed. **Scope:** large (touches all auth call sites) — a candidate deliverable OUT of the scheduled login/user-management audit above, not a quick fix.
...
- **WHY:** Consistency — entities are already relation-tier-filtered elsewhere; the graph-expansion edge path is the one spot relations leak past their own tier for local multi-user households.
