...
### 🔐 Full security audit — login & user management (SCHEDULED: after Notes + project-timeline)
**Trigger:** the operator wants a **full security audit of the login and user-management functionality** once the Notes + project-timeline (business-instance Phase 4) work is finished. This is a forcing-function item — pull it up the moment that track closes. It is the umbrella over the two auth debts below (SSO hand-off cutover, JWT/session storage), plus a fresh sweep, not just those.
...
- **WHAT REMAINS (the actual vuln closure):** the legacy URL-fragment token handler (`main.tsx::_consumeOidcHashHandoff`, `#access_token=` — implicit flow + token-injection sink) is **still present** and only removed at cutover. Cutover = (1) migrate every **emitter** to redirect with `?code=` instead of `#access_token=` — the emitter today is the **cross-repo Reva OIDC callback**; (2) enable `SSO_HANDOFF_ENABLED`; (3) delete the fragment handler. Also **un-wired**: `GET /api/auth/sso/start` + the "Sign in with…" PKCE initiation (Renfield's own `google`/`github`/`apple` redirect providers are `enabled=False`).
