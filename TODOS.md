...
- **#10 Presence / media-follow room-switch latency (~1–2 min)** — 🔨 **RSSI FILTER + MARGIN HYSTERESIS implemented** (`feat/presence-rssi-filter`): `_assign_room` now smooths per-room RSSI with an asymmetric EWMA (fast attack / slow release), decays a room the current satellite stops hearing, and switches on a **filtered enter-margin** instead of a scan count — the field-standard ESPresense/Bermuda approach. **A 1-scan "adaptive fast-switch" was tried first and REJECTED by two high-effort reviews** (6+ flip-flop holes across two designs); a web survey confirmed the fix is signal filtering, not a backend heuristic (and that ~10s is the realistic floor, set by phone advert rate). Legacy raw-mean + N-scan path preserved behind `presence_rssi_filter_enabled=false`. Bigger accuracy win deferred: **mmWave-occupancy + BLE-identity sensor fusion** (`docs/design/`, if pursued). See P2 detail.
