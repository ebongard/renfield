...
- 🟡 **(10) generative-UI widgets** — **first slice SHIPPED** (`feat/gen-ui-widgets`): a purpose-built **`weather` artifact kind** (`internal.weather_widget` → Open-Meteo MCP → current + forecast widget; `WEATHER_ENABLED`) + **agent-callable render tools** (`internal.render_table`/`render_list`, `services/widget_tools.py`) so a "list/table on request" comes back as a typed widget. The agent passes structured data as tool args (validated via `artifact_service` — still typed-JSON, never free-text parsing); `chat_handler._collect_tool_artifacts` rides it onto `_emit_turn_artifacts`. Rides `ARTIFACTS_TYPED_ENABLED` (live). **Remaining:** interactive/live device widgets (toggles, thermostat dial, presence map) — those need an artifact→action write-back channel (a bigger step than these read-only widgets), and the richer free-form widgets are gated on Lane B / option-(c) separate-origin.
