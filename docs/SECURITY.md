...
- **Short-lived WS-scoped token (M2 groundwork).** `POST /api/ws/token` returns a ~90s `scope:"ws"` access JWT (`WS_JWT_EXPIRE_SECONDS`) that `get_current_user`/`/auth/verify` reject on the REST/voice surfaces, so a WS token harvested from a proxy log is useless there. The browser-client switch to it (and removing the legacy `#access_token=` fragment handler) are deferred — see the SSO-cutover + M2-frontend items in TODOS.
