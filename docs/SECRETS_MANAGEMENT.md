...
| `internal_auth_verify_secret` | Shared-Secret, das `POST /api/internal/auth/verify` gated (`X-Verify-Secret` Header, constant-time). Schließt das unauth. Token-Validity-Oracle des Voice-Server-Verify-Endpoints (login audit 2026-08) | `secrets/internal_auth_verify_secret` (optional) | Backend (`Settings.internal_auth_verify_secret`). **Zweiseitig**: muss identisch am Voice-Server als `auth_callback_secret` bzw. Registry-`verify_secret` gesetzt sein — sonst 401t jeder Verify. `local`-Mode-Voice-Server rufen den Endpoint nie → dort gefahrlos |

**Hinweis zu optionalen Secrets**: `jellyfin_*`, `paperless_api_token`, `mail_primary_password`, `presence_webhook_secret`, `internal_auth_verify_secret` und die MCP-spezifischen API-Keys sind nur nötig, wenn die jeweilige Integration aktiviert ist. Fehlt eine Secret-Datei, bleibt das Feld in den Settings `None` und das Feature deaktiviert sich lautlos (kein Startup-Failure; `internal_auth_verify_secret` unset = Endpoint bleibt legacy-unauthenticated). **Ausnahmen**: `postgres_password`, `secret_key`, `default_admin_password` — diese werden vom Core gebraucht und müssen existieren.
