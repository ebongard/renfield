...
Auf der KG-Seite werden Entitäten **kanonisiert**: Schreibvarianten sammeln sich als `surface_forms` auf der kanonischen Zeile, eine Entität kann mehrere Rollen tragen (`entity_types`, z. B. Person + Musiker), und Relationen halten die Provenienz (`stated_by_user_id` — wer hat es gesagt). Ein periodischer **Reconciler** führt nachträglich entstandene Dubletten zusammen: same-tier mit hoher Ähnlichkeit automatisch, Cross-Tier/Grauzone/Schreibvariante eines Personennamens (`name_typo`, ein Edit innerhalb eines Tokens) als **Merge-Vorschlag** zur Owner-Review (`/brain/review`); ein abgelehnter Vorschlag wird nicht erneut gestellt. Zwei Riegel halten den Abgleicher ehrlich, weil die Einbettung sie nicht sehen kann: Personen mit unverwandten Namen werden gar nicht erst vorgeschlagen, und Entitäten **verschiedener Art** (ein Ort und die dort ansässige Firma stehen im selben Briefkopf und ähneln sich entsprechend) nur dann, wenn die Namen verwandt sind — dann als Vorschlag `cross_type`, nie als stille Faltung. „Verwandt“ meint dabei auch **zusammengeschrieben**: `ProductOwner` und `Product Owner` galten als unverwandt und fielen durch beide Riegel, obwohl es derselbe Name ist — sie werden jetzt vorgelegt (`name_tokenization`), ebenfalls nie still gefaltet. Beide Schreibweisen müssen dabei dieselben Wortteile ergeben: ein Verbundvorname wie `AnnaLena` enthält zwar `Anna`, ist aber ein anderer Mensch und bleibt getrennt. Eine Verschmelzung erhöht nie die Sichtbarkeit (Tier = MIN) — Details in [`CIRCLES.md`](CIRCLES.md#merge-invariante-structured-memory).
