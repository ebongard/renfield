...
# Gate je Meldungsart (BL-0424): NUR diese event_types dürfen durch das LLM
# (Auto-Dringlichkeit UND Anreicherung). Vorgabe = die technischen Absender
# (`ops_alert`, MCP-Health, geplante Aufgaben). Persönliche Meldungen —
# Erinnerungen, Fristen, HA-Ereignisse — bleiben wörtlich und erreichen kein
# Modell, auch wenn beide Schalter oben an sind. Kommaliste; leer = keine.
# Technische Absender (`ops_alert.notify_admin`) bieten jede Meldung zur
# Anreicherung an und überlassen bei aktivem Auto-Schalter die Dringlichkeit
# dem Klassifikator; bei ausgeschaltetem Schalter bleibt es bei `critical`.
...
# internal.paperless_dedupe: eine Dublette liegt vor, wenn ALLE Metadaten identisch
...
Personennamen clustern intrinsisch ≥ Schwelle (gemessen Erika~Anna 0.894), und
...
- **WARN (nicht fatal):** `AUTH_ENABLED=true` mit `CORS_ORIGINS='*'` (nur wenn Cookie-Mode AUS — mit Cookies ist es ein harter Fehler); `RENFIELD_ENV=production` mit `ALLOW_REGISTRATION=true` (ausdrücklich gesetzt — oder geerbt bei auth-off, wo es folgenlos ist).
...
- `WS_JWT_EXPIRE_SECONDS`: `90`
- `WS_RATE_LIMIT_ENABLED`: `true`
- `WS_RATE_LIMIT_PER_SECOND`: `50` (Audio-Streaming benötigt ~12.5/Sek.; Audio-Frames eines laufenden Satelliten-Zuges unterliegen nur dem Minutenbudget, `audio_end` kommt immer durch — #1284, siehe `docs/SECURITY.md`)
- `WS_RATE_LIMIT_PER_MINUTE`: `1000`
- `WS_MAX_CONNECTIONS_PER_IP`: `10`
...
PAPERLESS_AUDIT_REDERIVE_METADATA_AFTER_REOCR=true  # Nach erfolgreichem Re-OCR die Metadaten (Titel/Korrespondent/
                                 # Typ/Tags/Datum) aus dem jetzt guten Text neu ableiten + anwenden (sonst
                                 # bleiben sie aus dem alten garbled Text falsch)
