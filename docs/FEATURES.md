...
1. **Identität aus der Suche** (`mcp.paperless.search_documents`, `ordering=-created`, Sweep bis `SWEEP_CAP=500`): die Suche liefert seit `renfield-mcp-paperless` 1.10.0 auch `page_count`, sodass die volle Metadaten-Identität `(Korrespondent, Dokumenttyp, Datum, Titel, Seitenzahl)` **ohne** einen Aufruf pro Dokument feststeht (kein `get_document` — der `snippet` dient nur der Meldung, welche Gruppen erneute Scans waren).
2. **Fail-safe bei schwachen Metadaten**: Metadaten-Löschung setzt einen **nicht-leeren Titel UND eine vorhandene Seitenzahl** voraus. Fehlt eines (leerer Titel oder — z. B. bei älterer MCP — keine `page_count`), fällt die Kandidatengruppe auf den **byte-identischen** Vergleich zurück — ein unvollständiges Metadatum ist nicht „identisch", ein distinktes Dokument wird nie auf schwachem Signal gelöscht.
3. **Modus** `paperless_dedupe_metadata_match_enabled` (Default an). Aus = Legacy: nur **byte-identischer** OCR-Text (voller Textvergleich je Kandidatengruppe via `get_document`, `truncate=False`); ohne OCR-Text nicht beweisbar → nie gelöscht.
4. **Löschen**: das älteste Dokument (kleinste Paperless-ID) bleibt, jede weitere Kopie geht über `mcp.paperless.delete_document` in den **wiederherstellbaren Papierkorb** (Paperless-ngx 2.x — eine übereifrige Bereinigung ist rückgängig machbar).
