...
2. **Modus** `paperless_dedupe_metadata_match_enabled` (Default an). Aus = Legacy: nur **byte-identischer** OCR-Text (voller Textvergleich je Kandidatengruppe via `get_document`, `truncate=False`); ohne OCR-Text nicht beweisbar → nie gelöscht.
3. **Löschen**: das älteste Dokument (kleinste Paperless-ID) bleibt, jede weitere Kopie geht über `mcp.paperless.delete_document` in den **wiederherstellbaren Papierkorb** (Paperless-ngx 2.x — eine übereifrige Bereinigung ist rückgängig machbar).
