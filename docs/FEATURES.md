...
**Dublette = dasselbe Dokument, mehrfach abgelegt.** Es gilt als dasselbe, wenn **alle Metadaten identisch** sind (Korrespondent · Dokumenttyp · Erstelldatum · Titel · **Seitenzahl**) ODER der OCR-Text byte-identisch ist. Damit werden auch **erneut eingescannte Kopien** erkannt (gleiche Metadaten, leicht abweichender OCR-Text) — genau der Fall, den ein reiner Byte-Vergleich nie fand (z. B. ein Leasingvertrag, der dreimal abgelegt wurde). Ablauf:

1. **Ganzes Archiv durchsuchen** (`mcp.paperless.search_documents`, `ordering=-created`): da die MCP jede Suche auf 500 Treffer deckelt und erneut abgelegte Kopien ihr **Originaldatum** behalten (Duplikate sind also über den ganzen Korpus verstreut, nicht am neuesten Ende gebündelt), paginiert das Tool über `created_before`-Datumsfenster durch den **kompletten** Bestand (Dedup per ID). Das schließt die alte `SWEEP_CAP=500`-Lücke, bei der alles jenseits der neuesten 500 unsichtbar blieb.
2. **Identität aus der Suche**: seit `renfield-mcp-paperless` 1.10.0 liefert die Suche auch `page_count`, sodass die volle Metadaten-Identität `(Korrespondent, Dokumenttyp, Datum, Titel, Seitenzahl)` **ohne** einen Aufruf pro Dokument feststeht (kein `get_document` — der `snippet` dient nur der Meldung, welche Gruppen erneute Scans waren).
