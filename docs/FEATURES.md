...
- **Restart-sicherer Abschluss** (#658): Der Commit lädt async hoch und beendet in einem Hintergrund-Task (Consume pollen → deferred Metadaten-PATCH → Tracking-Zeile). Überlebt der Consume das ~300s-Poll-Fenster ODER startet der Pod mitten im Poll neu, ginge dieser In-Memory-Task verloren und PATCH/Tracking landeten still nie. Die Abschluss-Intent wird deshalb als `paperless_pending_finalize`-Zeile **vor** dem Task-Spawn persistiert; ein periodischer Reconciler (`services/paperless_finalize_reconciler.py`) führt verlorene Abschlüsse jenseits einer Karenz erneut aus (idempotent, Redis-Lease pro Zeile, UNIQUE-Constraint auf `paperless_upload_tracking.chat_upload_id` als Concurrency-Backstop). Ein noch konsumierendes Ergebnis erstattet seinen Versuch zurück (gesund-langsam ≠ Fehler) und wird nicht erneut angekündigt; nach `MAX_ATTEMPTS` echten Fehlern ODER dem Wall-Clock-Backstop (`GIVEUP_HOURS`) wird die Zeile geschlossen und **laut** geloggt (nie still verworfen). Env: `PAPERLESS_FINALIZE_RECONCILER_*` (siehe `docs/ENVIRONMENT_VARIABLES.md`).

### Watch-Folder-PDF → Simba-Review-Queue (xidra, `FOLDER_INGEST_SIMBA_ENABLED`, dark)
- **Kein Auto-Upload**: eine per Watch-Folder ingestete **PDF** (nur `source='folder_ingest'`) wird auf xidra zusätzlich als **Simba-Steuerportal-Kandidat** aufgenommen — aber NIE automatisch hochgeladen: die Übertragung an den Steuerberater ist **irreversibel** (das Portal verbietet Rückzug).
- **Review statt Automatik**: Der Post-Ingest-Hook (`simba_ingest_post_hook`) klassifiziert den Inhalt gegen eine stabile Taxonomie und legt eine PENDING-Zeile (`simba_ingest_proposals`) mit Kategorie/Typ-**Vorschlag** an. Der Owner bestätigt/editiert/verwirft sie auf `/brain/review` (`SimbaIngestReviewSection`, gated `simba_ingest_review_enabled`).
- **Bezeichnung**: Jede Review-Zeile zeigt zusätzlich ein editierbares **Bezeichnung**-Feld, vorbefüllt aus dem Dokumenttitel (`generated_title` → `title` → Dateiname-Stamm) und auf den erlaubten Simba-Zeichensatz sanitisiert (Buchstaben+Ziffern+Umlaute+Leerzeichen `. _ -`, max. 100). Die simba-MCP validiert die Bezeichnung und **wirft** bei unerlaubten Zeichen — ohne die renfield-seitige Sanitisierung würde ein Titel mit Gedankenstrich/Komma/Schrägstrich den Upload sprengen. Bearbeitete Eingabe gewinnt (ebenfalls sanitisiert); leer/nur-unerlaubt fällt auf den abgeleiteten Titel zurück. Zuvor kam die Bezeichnung beim Push über /brain/review **leer** an (der Review-Flow kannte kein Beschreibungsfeld).
- **Buchungszeitraum**: Jede Review-Zeile hat editierbare **Monat/Jahr**-Auswahlen (lokalisierte Monatsnamen), **vorbelegt mit dem Dokumentdatum** — aus dem Schicht-A-`rechnungsdatum` (→ weitere Datums-Fakten → Datum im generierten Titel), Fallback aktueller Monat, wenn kein Datum ableitbar ist (#1167; vorher immer aktueller Monat, wodurch z. B. eine März-Rechnung als August verbucht wurde). Die simba-MCP stempelt einen fehlenden Zeitraum **still** auf HEUTE — deshalb wird er immer angezeigt + gesendet (serverseitig validiert 1–12 / 2000–2100).
- **Bereits-in-Simba-Prüfung**: Vor dem irreversiblen Upload fragt `confirm()` Simba per `mcp.simba.list_transfers` ab, gefiltert nach Bezeichnung (Suchbegriff) **+ Kategorie + Typ — alles serverseitig** über die Grid-Widgets der Portal-Oberfläche (MCP ≥v1.0.8: kein Voll-Scan, kein 200-Zeilen-Fenster). Bei Treffer → `already_in_simba` (200, kein Fehler), KEIN Upload; die Zeile warnt mit dem vorhandenen Eintrag + Button **„Trotzdem übertragen"** (`force=true`). Best-effort — ein Prüf-Fehler blockiert nie. (Der Zeitraum-Datumsfilter des Portals ist NICHT automatisierbar; Bezeichnung + Kategorie + Typ genügt.)
- **Inline-Overlay statt Seitenwechsel**: Ein Klick auf **„An Simba senden"** auf einer Dokumentzeile öffnet ein **Overlay**, das den Vorschlag anlegt, Bezeichnung/Kategorie/Typ/Zeitraum vorbefüllt und die (unwiderrufliche) Übertragung **direkt dort** durchführt — mit einem gestalteten Zwei-Schritt-Bestätigen (kein `window.confirm`), ≥44px-Touch-Zielen und einer klaren Erfolgsmeldung. Abbrechen lässt den Vorschlag in der `/brain/review`-Queue liegen (dauerhafter Fallback; idempotent pro Dokument). Der Deep-Link `/brain/review#simba-{id}` springt weiterhin zur Queue-Zeile.
- **Kein Doppel-Upload**: `confirm()` ist **claim-before-act** (bedingtes `PENDING → UPLOADING`-UPDATE VOR dem irreversiblen `mcp.simba.upload_documents dry_run:false confirm:true`, `truncate=False`), sodass zwei gleichzeitige Bestätigungen (Doppelklick/Retry) nie doppelt hochladen — der Verlierer erhält 409. Nur bei echtem Landen (`uebertragen>0`) wird UPLOADED gesetzt, sonst zurück auf PENDING (erneut versuchbar).
- **Bestehende KB-Dokumente**: Der Hook feuert nur bei **neuen** Ordner-Dokumenten — ein bereits im Wissensspeicher liegendes Dokument wird beim Ingest per Content-Hash **dedupliziert** und erreicht den Hook nie. Zum Senden solcher Dokumente gibt es die Aktion **„An Simba senden"** auf jeder KB-Dokumentzeile (`/knowledge` + `/wissen/dokumente`), die das Inline-Overlay (siehe oben) öffnet: `POST /api/simba-ingest/from-document/{id}` → `create_proposal_for_document` legt den PENDING-Vorschlag an und liefert die Vorbefüll-Werte (owner/admin-gated — Owner = Atom-Owner, Atom-loses Dokument nur für Admins; idempotent). Übertragen/verwerfen geschieht im Overlay (oder später in der `/brain/review`-Queue). So speisen **zwei** Wege die Queue: neue Ordner-PDFs (automatisch) und bestehende KB-Dokumente (per Button/Overlay).
- **Zugriff**: Routen `GET/POST /api/simba-ingest…` sind auth-pflichtig und owner-scoped (Admin sieht Owner-lose Vorschläge). Das interaktive Chat-Pendant ist die Zwei-Tool-Brücke `internal.forward_attachment_to_simba` + `internal.simba_commit_upload`. Model `SimbaIngestProposal`, Migration `pc20260828b`. Siehe [FOLDER_INGEST.md](FOLDER_INGEST.md).
...
**Dokumentsuche nach Name & Inhalt** (immer an): Das Suchfeld auf `/knowledge` (bzw. `/wissen/dokumente`) findet ein Dokument über seinen **Namen** (auch den synthetisierten sprechenden Titel), seine **Fakten** oder seinen **Inhalt** — unabhängig von den 100 zuletzt gezeigten Einträgen der einfachen Liste. Zuvor war ein älteres oder beim Ingest dedupliziertes Dokument in der UI faktisch unauffindbar, wenn es aus dem Recency-Fenster gerutscht war. Drei Signale schlagen Kandidaten vor (Name-Volltext + Teiltreffer, Schicht-A-Fakten, semantische Chunks), Reciprocal Rank Fusion sortiert sie, und **ein einziges Circle-Sichtbarkeits-Gate** stellt sicher, dass nur zugängliche Dokumente erscheinen (die Namenssuche allein ist nicht kreis-gefiltert — das gemeinsame Gate ist die alleinige Sicht-Grenze, sodass kein verstecktes Dokument durchsickert). Leere Eingabe → die gewohnte Liste nach Aktualität. Mehrsprachig (DE/EN/FR/IT/ES/NL). *Später (PR2):* föderierte Suche zusätzlich über Paperless und (xidra) Simba.
...
## Geplante Aufgaben (Scheduled Tasks, #1137, Engine läuft immer; UI `scheduled_tasks_enabled`, opt-in/dark)
...
- **Whole-File-Konfidenz-Gate** (`PDF_SPLIT_AUTO_THRESHOLD`, Default 0.85):
  ...
  Dateien landen als Vorschlag auf **/brain/review** („PDF-Aufteilung
...
**Dublette = dasselbe Dokument, mehrfach abgelegt.** Es gilt als dasselbe, wenn die **Datei-Checksum identisch** ist (byte-identisches Original — das exakte Primärsignal, PASS 1; fängt Re-Ingest-Loop-Kopien auch bei driftenden Metadaten) ODER **alle Metadaten identisch** sind (Korrespondent · Dokumenttyp · Erstelldatum · Titel · **Seitenzahl**) ODER der OCR-Text byte-identisch ist (PASS 2, für **erneut eingescannte Kopien**: gleiche Metadaten, abweichender OCR — der Fall, den ein reiner Byte-Vergleich nie fand, z. B. ein dreimal abgelegter Leasingvertrag). Ablauf:

1. **Ganzes Archiv INDEX-UNABHÄNGIG aufzählen** (`mcp.paperless.list_all_documents`, `renfield-mcp-paperless` ≥1.11.0): DB-geordnet nach `id`, voll paginiert, mit `checksum`. Ein veralteter/partieller Paperless-Suchindex kann so **keine** Kopien mehr verstecken, und eine Duplikat-Gruppe mit >500 Kopien am **selben Erstelldatum** ist jetzt erreichbar (der alte `created_before`-Tagesfenster-Sweep blieb genau daran hängen — er ist nur noch Fallback, wenn die Antwort den `total_count`-Contract-Marker nicht trägt, z. B. eine ältere MCP; ein Guard akzeptiert einen solchen Teil-Sweep nie als „vollständig" → meldet nie fälschlich „sauber").
2. **Identität aus der Aufzählung**: `checksum` (PASS 1) + `(Korrespondent, Dokumenttyp, Datum, Titel, Seitenzahl)` (PASS 2) stehen **ohne** einen Aufruf pro Dokument fest (kein `get_document`; nur der byte-identische OCR-Fallback holt den Volltext).
3. **Fail-safe bei schwachen Metadaten**: Metadaten-Löschung setzt einen **nicht-leeren Titel UND eine vorhandene Seitenzahl** voraus. Fehlt eines (leerer Titel oder — z. B. bei älterer MCP — keine `page_count`), fällt die Kandidatengruppe auf den **byte-identischen** Vergleich zurück — ein unvollständiges Metadatum ist nicht „identisch“, ein distinktes Dokument wird nie auf schwachem Signal gelöscht.
4. **Modus** `paperless_dedupe_metadata_match_enabled` (Default an). Aus = Legacy: nur **byte-identischer** OCR-Text (voller Textvergleich je Kandidatengruppe via `get_document`, `truncate=False`); ohne OCR-Text nicht beweisbar → nie gelöscht.
5. **Rate-Limit-schonendes Löschen in Portionen**: die Paperless-MCP ist auf 60/min gedrosselt, daher löscht jeder Aufruf höchstens `paperless_dedupe_delete_batch` (Default 50) Kopien — jede mit **Retry/Backoff**, falls das Rate-Limit greift — und meldet, wie viele **noch verbleiben**; bei Rest einfach erneut aufrufen. Das älteste Dokument je Gruppe bleibt, jede weitere Kopie geht über `mcp.paperless.delete_document` in den **wiederherstellbaren Papierkorb**. Das Tool behauptet **nie**, das Archiv sei sauber, solange noch Kopien übrig sind (Fix des 2026-07-27-Bugs: ein ungedrosseltes Massenlöschen wurde zu ~226 von 317 vom Rate-Limit abgewiesen, während der Agent „316 gelöscht, sauber“ meldete).

`dry_run=true` meldet den vollständigen Duplikat-Umfang, ohne zu löschen. **Fail-closed**: bei aktiver Auth erfordert das Löschen einen authentifizierten **ADMIN**; ein unidentifizierter Turn (`user_permissions=None` — Geräte-/Satelliten-Token oder unerkannte Sprachstimme) wird abgelehnt (Bulk-Archiv-Löschung hat größere Tragweite als die reversiblen Wartungs-Tools). Auth aus (Einzel-Haushalt) überspringt den Gate. Ergänzt den admin-seitigen **Duplikate**-Tab (nur Anzeige) um die aktive Bereinigung.
