...
- **Built-ins**: **Paperless-Dedupe** (autonomes Aufräumen der Dubletten; self-gated auf `PAPERLESS_DEDUPE_RECONCILER_ENABLED`, ruft `mcp.paperless.dedupe_documents` — die Dedupe-Logik liegt im Paperless-MCP ≥1.12.0, das interaktive `internal.paperless_dedupe` ist ein Thin-Caller desselben Tools), **Paperless-Suchindex prüfen** (`paperless_index_health`, Fix B; dunkel hinter `PAPERLESS_INDEX_CHECK_ENABLED` und separat `PAPERLESS_INDEX_HEAL_ENABLED`: `mcp.paperless.search_index_health` prüft eine Seite Dokument-IDs aus der Datenbank gegen den Volltextindex, heilt fehlende per nicht-destruktivem Neu-Speichern — Paperless kennt kein REST-Reindex — und wirft bei nachgewiesener, ungeheilter Lücke, damit der Fehlserien-Alarm greift), `federation_audit_cleanup`, `upload_cleanup`.
