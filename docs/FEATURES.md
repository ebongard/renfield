...
- **Status/Phasen**: Phase 1 (Engine + Built-ins) + Phase 2 (REST + UI) implementiert; Phase 3 (Migration der restlichen ~20 `_schedule_*`) ausstehend — Whisper-Preload (Einmal-Job), Notification-Poller (persistente Verbindung) und Sub-Tick-Jobs bleiben legacy. Beim Migrieren gilt: die volle Runtime-Gate im Handler wiederholen (z. B. Obligation-Notifier/-Digest tragen ihre `proactive_enabled`-Gate nur im Wrapper, nicht in `scan_all_users`).
