...
- **Modell** (`notes`-Tabelle, Migration `pc20260720_notes`): NOT-NULL `atom_id` (via `AtomService.create_with_source` — geboren als Atom, wie `document_facts`), `owner_user_id`, `project_id` (4A-Timeline-Synergie), `title` (der `[[link]]`-Schlüssel, unique je Owner), Markdown-`body`, GENERATED mehrsprachige FTS `search_vector` (dense Embedding ist ein Follow-up). Registriert in `AtomService._table_for_atom_type` + `_source_id_for`, `circle_sql.notes_circles_filter`.
- **Retrieval**: `NoteRetrieval` (FTS, kreis-gefiltert) ist eine 8. Quelle in `PolymorphicAtomStore` (gated `notes_enabled`) → Notizen erscheinen in `/brain` / einer `/wissen`-Lens ohne Route-Änderung.
...
- **Frontend**: `/notes` (Anlegen + Liste + Inline-Bearbeiten + Löschen), `NoteLinksPanel` (Outgoing- + Backlink-Chips, Dangling gedämpft), Badge im `/brain`. **CRUD `/api/notes`** owner-gated 404, 409 bei Titel-Kollision. Dark by default. **Später (4B.3)**: Markdown-Vorschau, `[[ ]]`-Typeahead, Notizen-`/wissen`-Lens.
