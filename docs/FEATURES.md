...
- **Built-ins**: **Paperless-Dedupe** (autonomes Aufräumen der Dubletten; self-gated auf `PAPERLESS_DEDUPE_RECONCILER_ENABLED`, ruft `mcp.paperless.dedupe_documents` — die Dedupe-Logik liegt im Paperless-MCP ≥1.12.0, das interaktive `internal.paperless_dedupe` ist ein Thin-Caller desselben Tools), **Paperless-Suchindex prüfen** (`paperless_index_health`, Fix B; dunkel hinter `PAPERLESS_INDEX_CHECK_ENABLED` und separat `PAPERLESS_INDEX_HEAL_ENABLED`: `mcp.paperless.search_index_health` (Paperless-MCP ≥ 1.13.0) prüft eine Seite Dokument-IDs aus der Datenbank gegen den Volltextindex — eine Positivkontrolle bzw. ein gemerkter Nachweis macht auch komplett fehlende alte Seiten erkennbar —, heilt fehlende per leerem PATCH (Paperless kennt kein REST-Reindex, indexiert aber bei jedem Dokument-Update neu; es wird kein Feld gesetzt, aber `modified` ändert sich und „Dokument aktualisiert"-Workflows laufen, daher verweigert das MCP die Heilung bei aktiven solchen Workflows außer mit `PAPERLESS_INDEX_HEAL_ALLOW_WORKFLOWS`), gibt Dokumente nach `PAPERLESS_INDEX_HEAL_MAX_ATTEMPTS` Versuchen auf (direkter Alarm, Durchlauf geht weiter) und wirft bei nachgewiesener, ungeheilter oder blockierter Lücke, damit der Fehlserien-Alarm greift), `federation_audit_cleanup`, `upload_cleanup`.
...
- **Attribution**: ehrliche Pseudonyme ("Sprecher N") + Ein-Klick-Human-Labeling (`POST /api/meetings/{id}/relabel` → Re-Render → **Reindex in-place**, stabile `transcript_document_id`). Auto-Match an enrollte Sprecher wird nicht gebaut (Entscheidung 2026-09-20; anonyme Cross-Meeting-Fingerprints + Merge-on-Enroll decken den Bedarf).
...
- **Dark by default** (`MEETING_MINUTES_ENABLED=false`); **auf beiden Instanzen an** — auth-on-Instanz zuerst, Haushalt seit 2026-09-21 (BL-0402). Aus → Panel fehlt, Endpunkte antworten 404.
...
**Produzenten: Browser *und* Satellit.** Die Extraktion (Schritt 1) hängt am gemeinsamen Seam `services/turn_extraction.py`: sowohl der Chat-WebSocket (`chat_handler`, nach dem `done`-Frame) als auch der Satelliten-Sprachpfad (`satellite_handler._spawn_satellite_extraction`, nach dem Persistieren des Turns) planen dieselbe Hintergrund-Extraktion ein — nie im Turn selbst, Antwort/TTS/Wakeword werden dadurch nicht verzögert, und eine fehlschlagende Extraktion kann den Turn nicht kaputt machen. Bis dahin extrahierte ausschließlich der Browser-Pfad; über einen Satelliten Gesagtes wurde beantwortet, gespeichert und sofort vergessen. Turns mit fehlgeschlagener Tool-Aktion (`action_success is False`) werden auf beiden Pfaden übersprungen. **Gesprochene Turns werden nur bei erkanntem Sprecher extrahiert**: ohne Speaker→User-Zuordnung (`User.speaker_id`) läuft weder die Memory-Extraktion noch der `post_message`-Hook (KG + Plugins) — eine unzugeordnete Stimme im Raum darf nicht zum Gedächtnis einer Person werden. Ebenso wird ein **nicht persistierter** Turn übersprungen (keine Konversations-`session_id`): Erinnerungen, die zu keiner Konversation gehören, entstehen so gar nicht erst.
...
Radiale Live-Konstellation des laufenden Systems auf dem Wandtablet/-display — Kern (aktive Agenten-Rolle + Sprachzustand), Ring der Agenten-Rollen, Ring der MCP-Tools (gesund/eingeschränkt/ausgefallen), Ring der Räume/Satelliten (online + Belegung), Föderations-Peers als äußerer Bogen, plus der **Aktives-Subsystem-Puls** (welcher Renfield-Teil diesen Turn genutzt wurde). Rein lesend, inhaltsfrei, `kiosk.view`-gated (Rolle „Kiosk“: `kiosk.view` + `rooms.read`, gedacht für ein Gerätekonto; Admins kommen ebenfalls durch).
