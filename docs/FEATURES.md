...
**Lookup-Felder + Kalender:** Ansprechpartner, Typ und Ablage sind **Lookup-Felder**, vorgefüllt mit den vorhandenen Paperless-Werten (native `<datalist>` über den neuen Endpunkt `GET /api/admin/paperless-audit/taxonomy`) — in derselben Box lässt sich ein Bestandswert wählen ODER ein neuer eintippen, der beim Übernehmen angelegt wird (Ansprechpartner immer, Typ/Tags per `paperless_autocreate_*`-Flag; Ablage wählt nur aus Bestand, da Storage-Paths nicht auto-angelegt werden). Tags haben denselben Vorschlags-Lookup im Chip-Editor; das **Datum** öffnet ein natives Kalender-Widget (`<input type="date">`). Benutzerdefinierte Felder: Key/Value-Editor in einer pro-Zeile ausklappbaren Lade. (Native Controls bewusst — sie entkommen dem `overflow-x-auto`-Clipping der Tabelle und sind tastatur-/i18n-fest.)
