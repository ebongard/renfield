...
| D4 | Speaker recognition **off** on xidra (all four `SPEAKER_*` knobs) **plus** a code gate in `chat_handler` and the resolver. Opt-in later, only after a GDPR assessment (TODOS). |
