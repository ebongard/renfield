...
2. **Anonyme Aktuierung: Anonym-Rechtesatz (D-4a).** Heute lassen `mcp_client.py:2185-2198` und `bootstrap.py:544` `user_permissions=None` absichtlich durch (#690). Beschluss: auf dem Satellitenpfad wird `None` zu einer festen **Grant-Liste** für unerkannte Stimmen. Sie ist nicht die Rolle *Gast* (die hat keinen MCP-Zugriff) und muss auf dem **MCP-Pfad** wirken: Gerätesteuerung läuft dort als `mcp.homeassistant.*`-Intent über `mcp_manager.execute_tool(..., user_permissions)` → `check_permission` (`action_executor.py:361`, `mcp_client.py:2200-2234`), das den Konventions-Grant `mcp.<server>` verlangt — `ha.control` wird dort nicht gelesen (es gilt nur für die zwei Ansage-Werkzeuge in `bootstrap.py:506-509` und für REST). Deshalb: Liste = `mcp.homeassistant`, `mcp.dlna`, `mcp.radio`, `mcp.jellyfin`, `mcp.weather`, `rooms.read`, `ha.control` **plus** eine `tool_permissions`-Abbildung für den `homeassistant`-Server (Schaltwerkzeuge → `ha.control`, `call_service` → `ha.full`; heute hat der Server keine, `config/mcp_servers.yaml`), damit „Licht, Rollos, Musik ja — Dienste, Kamera, Schreibwerkzeuge nein“ auf dem MCP-Pfad tatsächlich gilt. `ha.control` in der Liste hält `internal.announce_in_room`/`broadcast_announcement` für anonyme Stimmen offen (heutiger Zustand, jetzt eine Entscheidung). Nebenwirkung, gewollt: sobald der anonyme Zug eine Liste statt `None` trägt, greifen auch die `internal.*`-Tore (`RAG_MANAGE`, `ADMIN` → verweigert) — `docs/INTERNAL_TOOLS.md` und `.claude/rules/internal-tools.md` werden mit dem Posten fortgeschrieben. Die Abnahme prüft „Licht ja, Kamera nein“ **über den MCP-Pfad**, nicht über REST (§11 Nr. 3).
3. **Gerätekonto „Haushalt“ ohne Gedächtnis (D-4b).** Das Extraktions-Tor prüft `user_id`, nicht die Sprechererkennung (`satellite_handler.py:152-157`, `conversation_memory_service.py:1114`): ein Gerätekonto, dessen `user_id` in `sat_user_id` läuft (nötig für Lesen bis Stufe 2 und den Rechtesatz), würde Erinnerungen aller Stimmen sammeln. Beschluss: Kennzeichen `users.is_device_account`; die Extraktion (`_spawn_satellite_extraction`, `_identity_scoped_write_denied`) schreibt für Gerätekonten **nie**; **Präsenz wird für Gerätekonten nie gebucht** (`satellite_handler.py:921-931` registriert heute Sprachpräsenz für `sat_user_id`); Schreibwerkzeuge (Erinnerungen anlegen, Scan anfordern, `action_executor.py:368-377`) sind nicht im Anonym-Rechtesatz — ein dennoch entstehender Datensatz gehört dem Gerätekonto und ist damit Haushaltsbestand, kein Leck; ein erkannter Sprecher ersetzt das Gerätekonto im Zug (auf dem Satellitenpfad, `satellite_handler.py:886-904`). Raumverläufe gehören dem Gerätekonto (§8.1; P0 Nr. 6 führt sie als geteilte Artefakte).
...
|---|---|---|
| **P0 Bau (dunkel)** | (1) PSK am Handshake (§6.1 Nr. 1); (2) Anonym-Rechtesatz + `tool_permissions` für `homeassistant` (Nr. 2); (3) `users.is_device_account` + Extraktions-/Präsenz-/Ersetzungstor (Nr. 3); (4) `kiosk.view` + Rolle (§6.2); (5) Adoption eigentümerloser Unterhaltungen aus (§4.2); (6) geteilte Verläufe nach Meeting-Datenmodell (§8.1, L); (7) Subsume mehrnutzerfähig + Validator (§8.2); (8) `bin/backfill_household_tiers.py` (§4.1); (9) Tests §11 | gewöhnliche PRs, alle dunkel |
...
Bauposten P0 (je ein PR, dunkel, reviewbar): (1) PSK am Handshake — **im Bau** (`SATELLITE_PSK_HANDSHAKE_ENABLED`, Token `sat.<id>.<secret>` nur im Header, Sperre vor bcrypt, Handler-Bindung), (2) Anonym-Rechtesatz + `tool_permissions` `homeassistant`, (3) Gerätekonto-Kennzeichen + Tore (Extraktion, Präsenz, Ersetzung), (4) `kiosk.view` + Rolle, (5) Adoption aus, (6) geteilte Verläufe nach Meeting-Datenmodell, (7) Subsume mehrnutzerfähig + Validator, (8) Backfill-Skript, (9) Filter-Tests mit ≥ 2 Nutzern. Danach `docs/runbooks/auth-on-cutover-household.md` (Vorflug §7, P1–P3 als Kommandofolge, Abnahme §11, Rückweg §10). Issue-Text für (6) mit Meeting-Verweis: `tasks/backlog/ISSUE_ACTIONS.md`. Mit den Posten fortzuschreiben: `.claude/rules/satellite-trust-ota.md`, `.claude/rules/internal-tools.md`, `docs/INTERNAL_TOOLS.md`, `docs/ACCESS_CONTROL.md` (Rolle Kiosk), `.claude/rules/obligations.md`; am Cutover: `docs/CIRCLES.md:36`, `.claude/rules/circles.md`.
