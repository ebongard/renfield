...
- **Merge-on-enroll (NEXT increment):** when a human relabels/enrolls a fingerprint to a real person, bind it (`speaker_id`) and back-propagate to prior meetings' clusters (cross-meeting merge). Conservative default: **prefer split over merge** (a wrong merge silently conflates two people — the magnet-hub failure class).
