...
- **T3/T4:** auth-off (`auth_enabled=false`) has no `user_id` to target; `owner=None` docs (null-KB/global-RAG) target nobody.
...
| Scan job finished (2026-09-14) | `services/scanner_jobs.handle_job_event` (after the outcome was written as an assistant message into the requesting conversation) | `{type:"scan_job_finished", reason:<done\|unrouted\|failed\|interrupted>, session_id}` → the requester (auth-off: `_ALL`). Frontend: `useUserEvents` dispatches a window event → `ChatContext` reloads the open thread only if the outcome landed in it (deferred while a turn streams) + `ScanJobToast` only in a tab that wrote into that conversation (`utils/tabConversations.ts`); every tab still refreshes the conversation list. Still content-free: the status plus the conversation's `session_id` — a routing key the recipient can already open (owner's sockets; in auth-off the one household), no job or document identity. The coalescer keys on `(target, type, session_id)`, so two conversations' events in one window are not merged. |
...
2. **Auth-off / single-user (T3):** `auth_enabled=false` ⇒ `authenticate_websocket` returns no user ⇒ the socket registers under the `_ALL` group, and `emit_documents_changed` forces `target=None` (both key on `auth_enabled`), so every event lands in `_ALL`. One household, no circle boundary — safe. **Edge config note (historical):** this used to key on a separate `ws_auth_enabled`, so "WS auth off while app auth on" was expressible and would fan every user's *content-free* refetch to all sockets. That mix no longer exists — one flag governs both surfaces, so the broad-fan case is now exactly the single-household case.
