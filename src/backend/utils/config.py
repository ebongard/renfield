...
class Settings(BaseSettings):
    ...
    # P3 text-similarity signal: a per-user halfvec cosine self-join on the
    # document content_embedding (mean of chunk embeddings) catches re-scans /
    # re-exports that share NO extracted identifier. Needs the content_embedding
    # backfilled first (bin/backfill_document_content_embeddings.py); high threshold
    # keeps distinct-but-similar docs (e.g. two invoices from one vendor) apart.
    ...
    # None with a concrete grant set — a POSITIVE list over every MCP server:
    # what is named is reachable, everything else (including read-only servers
    # like documents or calendar) is denied for an unrecognised voice.
    ...
    # Satellite Session Timeouts — gelesen NUR vom SatelliteManager (#1209).
    # Browser-/Web-Geräte (DeviceManager) haben KEINEN Kehraus mehr: sie
    # verlassen die Liste allein mit dem Socket-Schluss (BL-0502, #1277).
    # Sicherungsnetz für einen HÄNGENDEN Satelliten, kein zweiter Aufnahme-Deckel.
    ...
    device_heartbeat_timeout: float = 60.0  # Satellit ohne Heartbeat so lange → getrennt (nur SatelliteManager)
