"""Comprehensive functional tests for Paperless Audit (/admin/paperless-audit).
...
Specifically needs:
  - Audit dashboard shows stats (docs w/ confidence, needs-review count)
    - Trigger an audit run → job kicked off → completion surfaces
    - Review-queue items can be approved/fixed and the change persists to Paperless (use tests/e2e/helpers/paperless.py)
"""
...
class TestAdminPaperlessAuditRenders:
