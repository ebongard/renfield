"""Comprehensive functional tests for Integrationen (/admin/integrations).
...
Specifically needs:
  - List MCP servers → matches /api/admin/integrations
    - Toggle an integration on/off → server status reflects it within 60s refresh interval
    - Failed server surfaces an error banner (kill a server, verify UI detects)
"""
...
class TestAdminIntegrationsRenders:
