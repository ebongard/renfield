"""Comprehensive functional tests for Einstellungen (/admin/settings).
...
Specifically needs:
  - Feature-flag toggles match Settings() fields
    - Changing a flag via the UI persists (if edit is allowed) or surfaces read-only
    - Secret fields are masked (SecretStr — never leaked in HTML)
"""
...
class TestAdminSettingsRenders:
