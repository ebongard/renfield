"""Comprehensive functional tests for Zweites Gehirn (/brain).
...
Specifically needs:
  - Search the atom store with a known query → verify results include the seeded atom
    - Edit an atom's circle tier → verify the denormalized circle_tier column on document_chunks reflects the change
    - Filter by atom_type → UI state matches backend filtering
"""
