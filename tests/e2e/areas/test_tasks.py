"""Comprehensive functional tests for Aufgaben (/tasks).
...
Specifically needs:
  - Create a task → POST returns 200 → UI shows it
    - Mark complete → PATCH → DB state reflects it
    - Delete → UI and DB both gone
    - Reminder-scheduled task fires the proactive notification (if PROACTIVE_REMINDERS_ENABLED)
"""
